CREATE TABLE `audit_logs` (
  ...
  CONSTRAINT `fk_audit_logs_user`
    FOREIGN KEY (`user_id`) REFERENCES `authentication_users` (`id`) ON UPDATE CASCADE ON DELETE SET NULL
...
CREATE TABLE `audit_log_changes` (
  ...
  `created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  PRIMARY KEY (`id`),
  KEY `idx_audit_log_changes_audit_log_id` (`audit_log_id`),
  CONSTRAINT `fk_audit_log_changes_audit_log`
    FOREIGN KEY (`audit_log_id`) REFERENCES `audit_logs` (`id`) ON UPDATE CASCADE ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
