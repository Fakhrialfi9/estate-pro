...
export async function seedPropertyMatching(tx: SeedTransaction): Promise<void> {
  await tx.propertyPreference.upsert({
    ...
    update: {
      ...
      propertyTypeUuids: [seedUuid('property-type', 'RESIDENTIAL')],
      propertyCategoryUuids: [seedUuid('property-category', 'APARTMENT')],
      hardCriteria: { minBedrooms: 1, maxBedrooms: 3, minBathrooms: 1, parkingRequired: true },
      countryUuid: seedUuid('country', 'ID'),
      provinceUuid: seedUuid('province', 'JB'),
      cityUuid: seedUuid('city', 'BDG'),
      districtUuid: seedUuid('district', 'CB'),
      subdistrictUuid: seedUuid('subdistrict', 'DGO'),
      radiusKm: '10.00',
      latitude: '-6.8723000',
    ...
    create: {
      uuid: seedUuid('property-preference', SUBJECT_UUID),
  ...
  const recommendation = await tx.recommendation.create({
    data: {
      uuid: seedUuid('recommendation', `${SUBJECT_UUID}:1`),
      subjectType: SUBJECT_TYPE,
      subjectUuid: SUBJECT_UUID,
      preferenceVersion: 1,
      algorithmVersion: 1,
      source: 'GENERATED',
      generatedAt: SEED_REFERENCE_DATE,
      candidateCount: 1,
    },
  });
  const existing = await tx.recommendationItem.findFirst({ where: { recommendationId: recommendation.id, listingUuid: LISTING_UUID }, select: { id: true } });
  const item = existing
    ? await tx.recommendationItem.update({ where: { id: existing.id }, data: { propertyUuid: PROPERTY_UUID, rank: 1, score: '92.50', explanation: { budget: 'within range', location: 'preferred city', bedrooms: 'within range' } } })
    : await tx.recommendationItem.create({ data: { uuid: seedUuid('recommendation-item', `${recommendation.uuid}:1`), recommendationId: recommendation.id, propertyUuid: PROPERTY_UUID, listingUuid: LISTING_UUID, rank: 1, score: '92.50', explanation: { budget: 'within range', location: 'preferred city', bedrooms: 'within range' } } });
  await tx.recommendationHistory.create({ data: { uuid: seedUuid('recommendation-history', recommendation.uuid), recommendationId: recommendation.id, subjectType: SUBJECT_TYPE, subjectUuid: SUBJECT_UUID, source: 'GENERATED', preferenceVersion: 1, algorithmVersion: 1, candidateCount: 1, generatedAt: SEED_REFERENCE_DATE } });
