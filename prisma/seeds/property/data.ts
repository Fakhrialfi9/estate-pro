...
const PROPERTY_SEEDS = [
  ...
  ['kemang-townhouse', 'HOUSE', 'TOWNHOUSE', 0, 'Kemang Courtyard Townhouse', 'kemang-courtyard-townhouse', 5600000000, 3, 3, 240, 210, 3, 2, 2021, 5],
  ...
  ['senayan-retail', 'RETAIL', 'SHOPHOUSE', 0, 'Senayan Corner Shophouse', 'senayan-corner-shophouse', 7200000000, 0, 2, 300, 240, 3, 4, 2017, 8],
  ...
  ['cidadap-house', 'HOUSE', 'TOWNHOUSE', 1, 'Cidadap Hills Townhouse', 'cidadap-hills-townhouse', 3850000000, 3, 3, 200, 175, 2, 2, 2022, 2],
  ...
  ['tebet-townhouse', 'HOUSE', 'TOWNHOUSE', 0, 'Tebet Garden Townhouse', 'tebet-garden-townhouse', 4750000000, 3, 3, 220, 180, 3, 2, 2020, 10],
...
export const PROPERTY_FIXTURES = PROPERTY_SEEDS.map(([
  ...
  const businessCode = key === 'senayan-residence'
    ...
    : key === 'dago-apartment'
      ? 'PROP-DGO-001'
      : `PROP-${String(key).toUpperCase().replaceAll('-', '-')}`;
  const referenceNumber = key === 'senayan-residence'
    ? 'EST-SNY-001'
    : key === 'dago-apartment'
      ? 'EST-DGO-001'
      : `EST-${String(key).toUpperCase()}`;
