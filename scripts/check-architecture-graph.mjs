...
const SYSTEM_ROOT = path.join(MODULES_ROOT, 'system');
...
function moduleName(filePath) {
  ...
  return relative[0] === 'modules' && relative[1] ? relative[1] : null;
}
...
function systemLayer(filePath) {
  const relative = path.relative(SYSTEM_ROOT, filePath).split(path.sep);
  const layer = relative[0];
  return ['application', 'domain', 'infrastructure', 'presentation'].includes(layer)
    ? layer
    : null;
}

function validateSystemLayerDependency(source, target) {
  const sourceLayer = systemLayer(source);
  const targetLayer = systemLayer(target);
  ...
  return forbidden[sourceLayer].has(targetLayer)
...
if (cycles.length > 0) {
  ...
  for (const cycle of cycles) console.error(`  ${cycle.map(relativeSourcePath).join(' -> ')}`);
  process.exit(1);
}
...
for (const [source, dependencies] of graph) {
  ...
  for (const target of dependencies) {
    ...
    if (!targetIsPublicModule) {
      crossModuleViolations.push(`${relativeSourcePath(source)} -> ${relativeSourcePath(target)}`);
    }
  }
}
...
if (crossModuleViolations.length > 0) {
  ...
  for (const violation of crossModuleViolations) console.error(`  ${violation}`);
  process.exit(1);
}

const systemLayerViolations = [];
for (const [source, dependencies] of graph) {
  for (const target of dependencies) {
    const violation = validateSystemLayerDependency(source, target);
    if (violation) systemLayerViolations.push(violation);
  }
}

if (systemLayerViolations.length > 0) {
  console.error('Illegal SYSTEM layer dependency detected:');
  for (const violation of systemLayerViolations) console.error(`  ${violation}`);
  process.exit(1);
}

console.log(
  `Architecture graph check passed: ${files.length} TypeScript source files, no circular dependencies, no illegal cross-module internal imports, and no forbidden SYSTEM layer dependencies.`,
);
