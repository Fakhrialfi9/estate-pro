...
const ALLOWED_CROSS_MODULE_DEPENDENCIES = new Set([
  ...
  'src/modules/system/presentation/activity.controller.ts->src/modules/auth/security/jwt-auth.guard.ts',
  'src/modules/system/presentation/jobs.controller.ts->src/modules/auth/security/jwt-auth.guard.ts',
  'src/modules/system/presentation/notifications.controller.ts->src/modules/auth/security/jwt-auth.guard.ts',
  'src/modules/system/presentation/settings.controller.ts->src/modules/auth/security/jwt-auth.guard.ts',
]);
...
async function resolveRelativeImport(fromFile,specifier){if(!specifier.startsWith('.'))return null;let target=path.resolve(path.dirname(fromFile),specifier);if(target.endsWith('.js'))target=target.slice(0,-3)+'.ts';for(const candidate of [target,`${target}.ts`,path.join(target,'index.ts')]){try{if((await stat(candidate)).isFile())return path.normalize(candidate);}catch{continue;}}return null;}
...
const files=await collectTypeScriptFiles(SRC_ROOT);const graph=new Map(files.map(file=>[file,new Set()]));for(const file of files){const source=await readFile(file,'utf8');for(const specifier of sourceImportSpecifiers(source,file)){const target=await resolveRelativeImport(file,specifier);if(target&&graph.has(target))graph.get(file).add(target);}}
const visiting=new Set();const visited=new Set();const stack=[];const cycles=[];function visit(file){if(visiting.has(file)){const start=stack.indexOf(file);cycles.push([...stack.slice(start),file]);return;}if(visited.has(file))return;visiting.add(file);stack.push(file);for(const dependency of graph.get(file)??[])visit(dependency);stack.pop();visiting.delete(file);visited.add(file);}for(const file of files)visit(file);if(cycles.length>0){console.error('Circular dependency detected:');for(const cycle of cycles)console.error(`  ${cycle.map(relativeSourcePath).join(' -> ')}`);process.exit(1);}
const crossModuleViolations=[];for(const [source,dependencies] of graph){const sourceModule=moduleName(source);if(!sourceModule)continue;for(const target of dependencies){const targetModule=moduleName(target);if(!targetModule||targetModule===sourceModule)continue;if(ALLOWED_CROSS_MODULE_DEPENDENCIES.has(dependencyKey(source,target)))continue;const targetRelative=path.relative(SRC_ROOT,target).split(path.sep);const targetIsPublicModule=targetRelative.length===3&&targetRelative[0]==='modules'&&targetRelative[1]===targetModule&&targetRelative[2]===`${targetModule}.module.ts`;if(!targetIsPublicModule)crossModuleViolations.push(`${relativeSourcePath(source)} -> ${relativeSourcePath(target)}`);}}
if(crossModuleViolations.length>0){console.error('Illegal cross-module internal dependency detected:');for(const violation of crossModuleViolations)console.error(`  ${violation}`);process.exit(1);}console.log(`Architecture graph check passed: ${files.length} TypeScript source files, no circular dependencies, no illegal cross-module internal imports.`);