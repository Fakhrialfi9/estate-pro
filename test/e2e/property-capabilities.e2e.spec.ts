...
describe('Property capabilities E2E security boundary', () => {
  ...
  it(
    'requires authentication for amenity mutations and sensitive document/history endpoints',
    async () => {
  ...
  it(
    'rejects malformed nested property identifiers before reaching the business layer',
    async () => {
      await httpRequest(app)
  ...
  it(
    'keeps the public structured-data endpoint separate from authenticated property capability APIs',
    async () => {
      const response = await httpRequest(app)
        .get(
          '/api/v1/seo/public/property/not-a-real-property/structured-data',
        )
        .expect(404);

      expect(response.body).toHaveProperty(
        'message',
        'Public SEO resource not found',
      );
    },
  );
});
