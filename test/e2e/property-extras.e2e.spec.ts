...
describe('Property extras HTTP boundary',()=>{let app:INestApplication;
 ...
 it('rejects unauthenticated malformed resource paths at the security boundary',async()=>{await request(app.getHttpServer()).get('/api/v1/property/properties/not-a-uuid/utilities').expect(401);});
});
