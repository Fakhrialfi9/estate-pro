...
type LoginResponse = { accessToken: string; tokenType: 'Bearer'; expiresIn: number };
type MeResponse = { uuid: string; passwordHash?: string };
type SessionsResponse = { data: Array<{ id: string }> };
...
const bodyOf = <T>(response: SuperTestResponse): T => response.body as unknown as T;
...
describe('Authentication E2E', () => {
  ...
  beforeEach(async () => { await cleanup(); await createActiveUser(); });
  ...
  it('logs in, establishes a session, authenticates /me, then logs out the current session', async () => {
    ...
    const valid = await httpRequest().post('/api/v1/auth/login').set('X-Forwarded-For', '10.20.0.1').set('User-Agent', 'estate-pro-e2e').set('X-Request-ID', 'auth-login-success').send({ identifier: user.email, password: PASSWORD }).expect(201);
    const result = bodyOf<LoginResponse>(valid);
    expect(result.tokenType).toBe('Bearer');
    expect(result.accessToken).toEqual(expect.any(String));
    expect(result.expiresIn).toBeGreaterThan(0);
    const me = await httpRequest().get('/api/v1/auth/me').set('Authorization', `Bearer ${result.accessToken}`).expect(200);
    const meBody = bodyOf<MeResponse>(me);
    expect(meBody.uuid).toBe(userUuid);
    expect(meBody.passwordHash).toBeUndefined();
    const sessions = await httpRequest().get('/api/v1/auth/sessions').set('Authorization', `Bearer ${result.accessToken}`).expect(200);
    ...
    await httpRequest().post('/api/v1/auth/logout').set('Authorization', `Bearer ${result.accessToken}`).expect(201);
    await httpRequest().get('/api/v1/auth/me').set('Authorization', `Bearer ${result.accessToken}`).expect(401);
    expect(await prisma.auditLog.count({ where: { action: 'LOGIN_SUCCESS', userId: user.id } })).toBe(1);
    expect(await prisma.auditLog.count({ where: { action: 'LOGOUT', userId: user.id } })).toBe(1);
  });

  it('rejects invalid credentials without revealing account existence', async () => {
    const user = await prisma.authenticationUser.findUniqueOrThrow({ where: { uuid: userUuid } });
    await httpRequest().post('/api/v1/auth/login').send({ identifier: user.email, password: 'Wrong-Password-123!' }).expect(401);
    const unknown = await httpRequest().post('/api/v1/auth/login').send({ identifier: 'missing@example.com', password: 'Wrong-Password-123!' }).expect(401);
  ...
  it('rejects malformed and expired JWTs at the real authentication boundary', async () => {
    ...
    const expired = jwt.sign({ sub: userUuid, sid: randomUUID() }, { secret: config.getOrThrow<string>('auth.jwt.secret'), issuer: config.getOrThrow<string>('auth.jwt.issuer'), audience: config.getOrThrow<string>('auth.jwt.audience'), algorithm: config.getOrThrow<string>('auth.jwt.algorithm') as 'HS256', expiresIn: -1 });
  ...
  it('revokes an owned session through the session API and records the security event', async () => {
    const user = await prisma.authenticationUser.findUniqueOrThrow({ where: { uuid: userUuid } });
    const login = await httpRequest().post('/api/v1/auth/login').send({ identifier: user.email, password: PASSWORD }).expect(201);
    const token = bodyOf<LoginResponse>(login).accessToken;
    const sessions = await httpRequest().get('/api/v1/auth/sessions').set('Authorization', `Bearer ${token}`).expect(200);
    const sessionsBody = bodyOf<SessionsResponse>(sessions);
    const sessionId = sessionsBody.data[0]?.id;
    expect(sessionId).toMatch(/^\d+$/);
    await httpRequest().delete(`/api/v1/auth/sessions/${sessionId}`).set('Authorization', `Bearer ${token}`).expect(200);
