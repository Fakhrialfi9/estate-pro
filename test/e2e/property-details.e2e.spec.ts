...
type Body = {
  data?: { uuid?: string };
  items?: Array<{ uuid: string; sortOrder?: number }>;
};
...
async function cleanupProperties() {
  ...
  await prisma.property
    .deleteMany({ where: { id: { in: [propertyId].filter(Boolean) } } })
    .catch(() => undefined);
  await prisma.facility
    .deleteMany({ where: { uuid: facilityUuid } })
    .catch(() => undefined);
  await prisma.propertyCategory
    .deleteMany({ where: { id: categoryId } })
    .catch(() => undefined);
  await prisma.propertyType
    .deleteMany({ where: { id: typeId } })
    .catch(() => undefined);
}
  ...
  afterAll(async () => {
    ...
    await prisma.authorizationUserRole.deleteMany({
      where: {
        userId: {
          in: (
            await prisma.authenticationUser.findMany({
