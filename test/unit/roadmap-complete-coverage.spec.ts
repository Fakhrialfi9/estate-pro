...
describe('roadmap complete coverage', () => {
  ...
  it('covers security guards including generic denial fallback', async () => {
    ...
    const authenticated = new AuthenticatedAccessGuard(verifier as never, sessions as never);
    ...
    const authorization = {
      resolve: vi.fn().mockResolvedValue({ userUuid: uuid, permissionCodes: ['properties.read'], roleCodes: ['agent'] }),
  ...
  it('covers audit log service success, redaction mapping and failure isolation', async () => {
    ...
    await expect(service.list({ page: 1, limit: 20, resourceType: undefined, actorUuid: undefined } as never)).resolves.toMatchObject({ total: 1 });
  ...
  it('covers JWT, password and refresh-token error branches', async () => {
    ...
    const config = new ConfigService({
      auth: { jwt: { secret, issuer: 'estate-pro-api', audience: 'estate-pro-client', algorithm: 'HS256', expiresIn: '15m' } },
    });
  ...
  it('covers session lifecycle branches and security-event reason mapping', async () => {
    ...
    const service = new SessionService(repo as never, audit as never, refresh as never);
  ...
  it('covers two-factor service lifecycle and failure branches', async () => {
    ...
    const jwt = {
      ...
      verifyMfaChallenge: vi.fn().mockResolvedValue({ sub: uuid, challengeId: 'challenge-id' }),
    };
    const hasher = {
    ...
    const valid = {
      ...
      graph: {
        ...
        nodes: [
          ...
          { id: 'action', type: 'ACTION', actionType: 'NOTIFY', input: {}, maxAttempts: 3, timeoutMs: 1000 },
        ],
  ...
  it('covers automation action execution retryability and domain lifecycle helpers', async () => {
    ...
    const definition = parseDefinition({
      trigger: { type: 'ENTITY_CREATED', entityType: 'LEAD' },
      graph: { entryNodeId: 'trigger', nodes: [{ id: 'trigger', type: 'TRIGGER', trigger: { type: 'ENTITY_CREATED', entityType: 'LEAD' } }], edges: [] },
    });
  ...
  it('covers automation service orchestration branches', async () => {
    ...
    await expect(service.createSlaPolicy(uuid, { durationMinutes: 60, targetEntityType: 'LEAD' }, uuid)).resolves.toBeDefined();
    await expect(service.createSlaPolicy(uuid, { durationMinutes: 0 }, uuid)).rejects.toBeInstanceOf(BadRequestException);
    await expect(service.listNotifications({ userUuid: '', page: 1, limit: 10, unreadOnly: false })).rejects.toBeInstanceOf(BadRequestException);
    await expect(service.markNotificationRead('notification-1', '')).rejects.toBeInstanceOf(BadRequestException);
