...
const passwordConfig = {
  getOrThrow: (key: string) => key === 'auth.passwordHashing'
    ? { memoryCost: 19456, timeCost: 2, parallelism: 1, hashLength: 32 }
    : key === 'auth.passwordReset.tokenTtlMinutes' ? 15 : undefined,
};

describe('credential security', () => {
  it('enforces a practical password policy and confirmation', () => {
    const policy = new PasswordPolicy();
    expect(policy.validate('short1A')).toEqual(expect.objectContaining({ valid: false }));
    expect(policy.validate('SecurePassword123')).toEqual({ valid: true });
    expect(policy.validate('123456789012')).toEqual(expect.objectContaining({ valid: false }));
    expect(() => policy.assertConfirmation('SecurePassword123', 'SecurePassword124')).toThrow();
  });

  it('hashes with Argon2id, verifies securely, and detects invalid passwords', async () => {
    const hasher = new PasswordHasherService(passwordConfig as unknown as ConfigService);
  ...
  it('changes password only after current-password verification and never returns a hash', async () => {
    const repository = {
      findByUserUuid: vi.fn().mockResolvedValue({ userUuid, passwordHash: 'old-hash' }),
    ...
    expect(repository.updatePassword).toHaveBeenCalledWith(userUuid, 'new-argon2-hash', expect.any(Date));
  ...
  it('keeps reset requests generic for existing and non-existing identities', async () => {
    ...
    const credentials = {
      findByUserUuid: vi.fn().mockResolvedValue({ userUuid, passwordHash: 'hash' }),
    ...
    await expect(service.requestByEmail('missing@example.com')).resolves.toEqual({ accepted: true });
    expect(credentials.createResetToken).not.toHaveBeenCalled();

    vi.mocked(users.findByEmail).mockResolvedValue(user as never);
    await expect(service.requestByEmail('member@example.com')).resolves.toEqual({ accepted: true });
    expect(credentials.createResetToken).toHaveBeenCalledWith(
      userUuid,
      expect.stringMatching(/^[a-f0-9]{64}$/),
      expect.any(Date),
    );
    expect(delivery.deliver).toHaveBeenCalledWith(expect.objectContaining({ token: expect.any(String) }));
  });
  ...
  it('uses the repository atomic reset contract and rejects token replay', async () => {
    ...
    const hasher = { hash: vi.fn().mockResolvedValue('new-argon2-hash') } as unknown as PasswordHasherService;
    ...
    await expect(service.reset(token, 'ResetSecurePassword123', 'ResetSecurePassword123')).resolves.toBeUndefined();
    await expect(service.reset(token, 'ResetSecurePassword123', 'ResetSecurePassword123')).rejects.toThrow(
