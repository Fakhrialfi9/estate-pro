...
describe('SEO policy', () => {
  ...
  it('normalizes and deduplicates keywords', () => {
    expect(normalizeKeywords([' Foo ', 'foo', 'bar'])).toEqual([
      'Foo',
      'foo',
      'bar',
    ]);
  ...
  it('builds only same-origin canonical URLs', () => {
    expect(
      buildCanonicalUrl('https://example.com', '/properties/a?x=1#hash'),
    ).toBe('https://example.com/properties/a');
    expect(() =>
  ...
  it('excludes unpublished resources from sitemap eligibility', () => {
    ...
    expect(
      isPubliclyIndexable(resource({ seo: { robots: 'noindex,follow' } })),
    ).toBe(false);
  });
