...
describe('property extras coverage', () => {
  it('covers Money validation, minor units and rounding branches', () => {
    ...
    expect(() => new Money('12', 'idr')).toThrow(
      'Money currency must be a 3-letter ISO-style code',
    );
  ...
  it('covers utility invariants and boundary failures', () => {
    ...
    ).toThrow('waterBackupSource must differ');
    expect(() => validateUtilityInvariants({ internetFiber: true })).toThrow(
      'internetProviders is required',
    );
    expect(() =>
      validateUtilityInvariants({ backupPowerType: 'GENERATOR' }),
    ).toThrow('backupPowerCapacityKva is required');
    expect(() =>
      validateUtilityInvariants({
        backupPowerType: 'NONE',
        backupPowerCapacityKva: '1',
      }),
    ).toThrow('backupPowerCapacityKva must be empty');
    expect(() =>
      validateUtilityInvariants({ internetProviders: ['ok', '   '] }),
    ).toThrow('invalid values');
    expect(() =>
      validateUtilityInvariants({
        internetProviders: Array.from({ length: 21 }, () => 'provider'),
      }),
    ).toThrow('invalid values');
    expect(() =>
    ...
    expect(() =>
      validateUtilityInvariants({ electricityCapacityKva: '999999999' }),
    ).toThrow();
    expect(() =>
      validateUtilityInvariants({ backupPowerCapacityKva: '999999999' }),
    ).toThrow();
  });
  ...
  it('covers legal and certificate validation rules', () => {
    ...
    expect(() =>
      validateCertificateDates('2026-02-01', '2026-02-01'),
    ).not.toThrow();
  });
  ...
  it('covers financial, SEO and media invariant branches', () => {
    ...
    expect(() =>
      validateSeoInvariants('villa-bali', {
        ...
        canonicalUrl: 'https://example.com/villa-bali',
      }),
    ).not.toThrow();
    expect(() =>
      validateSeoInvariants('villa-bali', { title: 'x'.repeat(61) }),
    ).toThrow('SEO title');
    expect(() =>
      validateSeoInvariants('villa-bali', {
        description: 'x'.repeat(161),
      }),
    ).toThrow('SEO description');
    expect(() =>
      validateSeoInvariants('villa-bali', {
