...
describe('AnalyticsService coverage', () => {
  it('normalizes query defaults, bounds, optional filters, and rejects invalid ranges', () => {
    ...
    expect(() => service.normalizeQuery({ from: 'invalid', to: 'invalid' })).toThrow(AnalyticsInvalidQueryException);
    expect(() => service.normalizeQuery({ from: '2024-01-01', to: '2026-01-01' })).toThrow('cannot exceed');
  });
  ...
  it('enforces revenue and forecast permissions and calculates forecast', async () => {
    const dto = { from: '2026-01-01T00:00:00.000Z', to: '2026-01-03T00:00:00.000Z' };
    const deniedRevenue = new AnalyticsService(makeQueries(), makePolicy(false, true, true) as never);
    await expect(deniedRevenue.salesAndRevenue(dto, user())).rejects.toThrow(AnalyticsScopeException);
    const deniedForecast = new AnalyticsService(makeQueries(), makePolicy(true, false, true) as never);
  ...
  it('exports supported reports to CSV, including escaping and empty results', async () => {
    ...
    const emptyService = new AnalyticsService(noRows, makePolicy(true, true, true) as never);
    const empty = await emptyService.exportCsv(dto, user(), 'pipeline');
    expect(empty.content).toBe('');
    await expect(new AnalyticsService(makeQueries(), makePolicy(true, true, false) as never).exportCsv(dto, user(), 'leads')).rejects.toThrow(AnalyticsScopeException);
  });
  ...
  it('normalizes unexpected query failures and timeout conditions', async () => {
    ...
    await expect(failingService.leads({ from: '2026-01-01', to: '2026-01-03' }, user())).rejects.toThrow(AnalyticsUnavailableException);

    vi.useFakeTimers();
    const pending = new Proxy({}, { get: () => vi.fn(() => new Promise(() => undefined)) }) as AnalyticsQueryPort;
    const pendingService = new AnalyticsService(pending, makePolicy() as never);
    const promise = pendingService.leads({ from: '2026-01-01', to: '2026-01-03' }, user());
