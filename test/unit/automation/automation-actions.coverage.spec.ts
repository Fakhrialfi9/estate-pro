...
describe('automation actions', () => {
  it('covers AssignLeadAction success/inactive/invalid/dependency error', async () => {
    ...
    const action = new AssignLeadAction(crm as never, users);
    ...
    const broken = new AssignLeadAction({ assignLead: vi.fn() } as never, {
      getUser: vi.fn().mockRejectedValue(failed),
    });
    await expect(
  ...
  it('covers CreateActivityAction defaults, optionals, dates, and invalid values', async () => {
    ...
    await expect(
      action.execute({ assigneeUserUuid: 'bad' }, { uuid: leadUuid }, actor),
    ).rejects.toBeInstanceOf(BadRequestException);
  });

  it('covers EnqueueCommunicationAction allowed/denied/unsafe/optional paths', async () => {
    const crm = {
      enqueueCommunication: vi.fn().mockResolvedValue({ uuid: undefined }),
    };
  ...
  it('covers NotifyAction success, inactive, unsafe, defaults, and validation', async () => {
    ...
    const action = new NotifyAction(repo as never, users);
    ...
    await expect(
      action.execute({ userUuid, title: 'Title', body: 'Body' }, {}),
  ...
  it('covers EscalateAction inactive/missing/context/optional activity paths', async () => {
    ...
    const action = new EscalateAction(crm as never, users);
