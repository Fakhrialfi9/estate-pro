...
describe('automation actions', () => {
  ...
  it('covers CreateActivityAction defaults, optionals, dates, and invalid values', async () => {
    ...
    await expect(
      action.execute({}, { uuid: leadUuid }, actor),
    ).resolves.toMatchObject({ success: true, reference: 'activity-1' });
  ...
  it('covers EnqueueCommunicationAction allowed/denied/unsafe/optional paths', async () => {
    ...
    await expect(
      action.execute({ channel: ' email ', body: 'Hello' }, { uuid: leadUuid }, actor),
  ...
  it('covers NotifyAction success, inactive, unsafe, defaults, and validation', async () => {
    ...
    await expect(
      action.execute({ userUuid: 'bad', title: 'Title', body: 'Body' }, {}, actor),
    ).rejects.toBeInstanceOf(BadRequestException);
  });
  ...
  it('covers EscalateAction inactive/missing/context/optional activity paths', async () => {
    ...
    await expect(
      action.execute({ userUuid }, { uuid: leadUuid, entityType: 'LEAD' }, actor),
    ).resolves.toMatchObject({
      success: true,
