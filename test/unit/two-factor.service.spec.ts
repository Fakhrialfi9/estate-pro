...
function createHarness() {
  ...
  const repository = {
    ...
    createPending: vi.fn(
      async ({ secretEncrypted }: { secretEncrypted: string }) => {
        encryptedSecret = secretEncrypted;
        lockedUntil = null;
      },
    ),
    enable: vi.fn(
      async ({ lastUsedTimeStep }: { lastUsedTimeStep: bigint }) => {
        enabled = true;
    ...
    recordFailedVerification: vi.fn(async () => {
      lockedUntil = new Date(Date.now() + 900000);
    }),
    recordSuccessfulVerification: vi.fn(
      async ({ timeStep }: { timeStep: bigint }) => {
        if (lastStep !== null && timeStep <= lastStep) return false;
        lastStep = timeStep;
        lockedUntil = null;
        return true;
      },
  ...
  const recovery = {
    replaceAll: vi.fn(async (_user: string, hashes: readonly string[]) => {
    ...
    findUnused: vi.fn(async () =>
      recoveryHashes
        .filter((item) => !item.used)
        .map(({ id, codeHash }) => ({ id, codeHash })),
    ),
    markUsed: vi.fn(async (id: bigint) => {
  ...
  const hasher = {
    ...
    verify: vi.fn(
      async (hash: string, value: string) => hash === `hash:${value}`,
    ),
  };
  const jwt = {
    issueMfaChallenge: vi.fn(async () => 'challenge'),
  ...
  const credentials = {
    findByUserUuid: vi.fn(async () => ({ passwordHash: 'hash:password' })),
  };
