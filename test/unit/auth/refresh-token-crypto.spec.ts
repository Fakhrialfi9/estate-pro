...
} from '../../../src/modules/auth/application/services/refresh-token-crypto.service.js';
...
describe('RefreshTokenCryptoService', () => {
  ...
  it('generates unique token values', () => {
    const service = new RefreshTokenCryptoService();
    expect(
      new Set(Array.from({ length: 100 }, () => service.generate())).size,
    ).toBe(100);
  });
  ...
  it('supports constant-time digest equality', () => {
    ...
    expect(
      service.equalsDigest(digest, service.digest(service.generate())),
    ).toBe(false);
  });
});
