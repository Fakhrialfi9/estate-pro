...
const dependencies = () => {
  const repo = {
    ...
    listTargets: vi.fn().mockResolvedValue([
      {
        uuid: 'target-1',
        metricType: 'propertyAssignments',
        targetValue: '10',
        status: 'ACTIVE',
      },
    ]),
  ...
  const propertyContext = {
    getContext: vi.fn().mockResolvedValue({
      countryUuid: 'country-1',
      provinceUuid: 'province-1',
      cityUuid: null,
      districtUuid: null,
      subdistrictUuid: null,
    }),
  };
...
describe('agent management roadmap coverage', () => {
  it('covers create eligibility failure, inactive specialization and update capacity branch', async () => {
    ...
    d.users.getUser.mockResolvedValueOnce({
      uuid: userUuid,
      isActive: false,
      status: 'disabled',
      deletedAt: null,
  ...
  it('covers assignment permission, capacity rejection, target lifecycle and performance KPIs', async () => {
    const d = dependencies();
    d.authorization.assertPermissions.mockImplementationOnce(() => {
      throw new Error('denied');
    });
    await expect(
      d.service.assign('property-1', agentUuid, actor),
    ).rejects.toBeInstanceOf(ForbiddenException);
    ...
    await expect(
      d.service.updateTarget('missing', { targetValue: '20' } as never, actor),
    ).rejects.toBeInstanceOf(NotFoundException);
    d.repo.findTarget.mockResolvedValueOnce({
      uuid: 'target-1',
      targetValue: '20',
      status: 'CLOSED',
    });
  ...
  it('covers findCandidates with property context, permission fallback, filtering and missing property', async () => {
    ...
    await expect(
      d.service.findCandidates({ propertyUuid: 'missing' }, actor),
    ).rejects.toBeInstanceOf(NotFoundException);
    ...
    await expect(
      d.service.findCandidates({ regionUuids: ['region-1'] }, actor),
    ).resolves.toBeDefined();

    d.repo.listProfiles.mockResolvedValueOnce([
      makeAgent({ status: 'ARCHIVED' }),
    ]);
    await expect(
      d.service.findCandidates({ regionUuids: ['region-1'] }),
    ).resolves.toEqual([]);
  });
});
