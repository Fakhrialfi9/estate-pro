...
} from '../../../src/modules/system/domain/errors/system.errors.js';
...
describe('SystemSettingsService boundary coverage', () => {
  ...
  it('rejects unknown keys and missing non-default stored values', async () => {
    ...
    await expect(service.get('unknown.setting')).rejects.toBeInstanceOf(
      NotFoundException,
    );
    repository.get.mockResolvedValueOnce(null);
    await expect(
      service.get('system.default_page_size'),
    ).resolves.toMatchObject({ value: 25 });
    repository.get.mockResolvedValueOnce({ value: undefined });
    await expect(
      service.get('system.public_status_url'),
    ).resolves.toMatchObject({ value: 'https://status.example.com' });
  });
  ...
  it('validates mutable settings, handles optimistic conflicts, and records audit/activity', async () => {
    ...
    await expect(
      service.update('unknown.setting', '1', 'actor'),
    ).rejects.toBeInstanceOf(NotFoundException);
    await expect(
      service.update('system.default_page_size', '', 'actor'),
    ).rejects.toBeInstanceOf(BadRequestException);
    await expect(
      service.update('system.default_page_size', 'nope', 'actor'),
    ).rejects.toBeInstanceOf(BadRequestException);
    await expect(
      service.update('system.maintenance_mode', 'maybe', 'actor'),
    ).rejects.toBeInstanceOf(BadRequestException);
    await expect(
      service.update('system.public_status_url', 'ftp://example.com', 'actor'),
    ).rejects.toBeInstanceOf(BadRequestException);

    repository.upsert.mockRejectedValueOnce(
      new SystemSettingConflictError('Version conflict'),
    );
    await expect(
      service.update('system.default_page_size', '50', 'actor', 1),
    ).rejects.toBeInstanceOf(ConflictException);
    repository.upsert.mockRejectedValueOnce(
      new SystemSettingImmutableError('Immutable'),
    );
    await expect(
      service.update('system.default_page_size', '50', 'actor', 1),
    ).rejects.toBeInstanceOf(BadRequestException);
