...
const actor = { actorUuid: uuid, permissions: ['crm.contacts.read'], requestId: 'r1' };
const preference: PropertyPreferenceState = {
  ...
  budget: { currency: 'IDR', frequency: 'TOTAL', min: '100', max: '200', tolerancePercent: 10 },
  specification: { bedrooms: { min: 2, max: 4 }, bathrooms: { min: '1', max: '3' }, areaSqm: { min: '80', max: '150' }, parkingSpaces: { min: 1, max: 2 }, furnishedStatus: 'FULLY_FURNISHED', condition: 'GOOD' },
  hardCriteria: [],
};
const candidate = (overrides: Partial<MatchCandidate> = {}): MatchCandidate => ({
  ...
  expiresAt: null,
  price: { currency: 'IDR', priceType: 'TOTAL', minPrice: '120', maxPrice: '180' },
  location: { countryUuid: uuid, latitude: 0.01, longitude: 0.01 },
  specification: { bedrooms: 3, bathrooms: '2', buildingAreaSqm: '120', parkingSpaces: 1, furnishedStatus: 'FULLY_FURNISHED', condition: 'GOOD' },
  ...overrides,
});
...
describe('property matching phase 9', () => {
  it('covers MatchingRuleService defaults, validation, audit and stale versions', async () => {
    ...
    await service.update(uuid, 1, { name: 'Updated', weights: { transactionType: 10 }, hardCriteria: ['budget'], minimumScore: 50 }, uuid2);
    ...
    await expect(service.update(uuid, 1, {}, uuid2)).rejects.toBeInstanceOf(ConflictException);
  });

  it('covers MatchingEngine hard criteria, weighted scoring, budget/location/specification/behavior and sorting', () => {
    const engine = new MatchingEngine();
    const results = engine.evaluate(preference, [candidate(), candidate({ listingUuid: uuid3, propertyTypeUuid: uuid3 })], new Map([[uuid2, signal]]), {
    ...
    const hardPref = { ...preference, hardCriteria: ['budget', 'location'] as const };
    expect(engine.evaluate(hardPref, [candidate({ price: { currency: 'USD', priceType: 'TOTAL', minPrice: '1', maxPrice: '2' } })], new Map(), DEFAULT_MATCHING_RULE)).toEqual([]);
    expect(engine.evaluate(hardPref, [candidate({ location: { countryUuid: uuid2, latitude: 0, longitude: 0 } })], new Map(), DEFAULT_MATCHING_RULE)).toEqual([]);
    expect(engine.evaluate(preference, [candidate({ price: null })], new Map(), DEFAULT_MATCHING_RULE)).toEqual([]);
    expect(engine.evaluate({ ...preference, budget: undefined }, [candidate({ specification: null, location: null })], new Map(), DEFAULT_MATCHING_RULE)).toBeDefined();
    expect(engine.evaluate({ ...preference, specification: { bedrooms: { min: 5 }, parkingSpaces: { min: 5 }, bathrooms: { min: '10' }, areaSqm: { min: '1000' } } }, [candidate()], new Map(), DEFAULT_MATCHING_RULE)).toEqual([]);
  });
  ...
  it('covers PropertyPreference value cloning and every validation edge', () => {
    ...
    expect(() => PropertyPreference.assertLocation({ countryUuid: 'bad' })).toThrow();
    expect(() => PropertyPreference.assertBudget({ currency: 'IDR', frequency: 'TOTAL', min: '1.001' })).toThrow();
    expect(() => PropertyPreference.assertSpecification({ parkingSpaces: { min: -1 } })).toThrow();
  });
  ...
  it('covers PropertyMatchingService preference lifecycle, match/generate/history/feedback and access fallbacks', async () => {
    ...
    repository.findPreference = vi.fn().mockResolvedValue({ ...preference, status: 'ARCHIVED' });
    await service.createPreference('USER', uuid, preference, actor);
    repository.findPreference = vi.fn().mockResolvedValue(preference);
    await expect(service.createPreference('USER', uuid, preference, actor)).rejects.toBeInstanceOf(ConflictException);
    await service.updatePreference('USER', uuid, 1, preference, actor);
    await service.archivePreference('USER', uuid, 1, actor);
    repository.findPreference = vi.fn().mockResolvedValue({ ...preference, status: 'ARCHIVED', version: 1 });
    await service.restorePreference('USER', uuid, 1, actor);
    await service.match('USER', uuid, { minScore: 0, page: 0, limit: 100 }, actor);
    await service.generate('USER', uuid, 'GENERATED', { minScore: 0, limit: 100 }, actor);
    await service.getLatest('USER', uuid, actor);
    await service.getHistory('USER', uuid, 0, 100, actor);
    await service.submitFeedback({ recommendationItemUuid: uuid, subjectType: 'USER', subjectUuid: uuid, propertyUuid: uuid2, listingUuid: uuid3, feedback: 'INTERESTED' }, actor);
