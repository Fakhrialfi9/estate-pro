...
import type { AuthenticationSecurityRepository, AuthenticationSecurityState, AuthenticationLockoutPolicy } from '../../src/modules/auth/domain/repositories/authentication-security.repository.js';
...
import type { SecurityAuditRepository, SecurityAuditEvent } from '../../src/modules/auth/domain/repositories/security-audit.repository.js';
...
const user = (overrides: Partial<UserEntity> = {}): UserEntity => ({
  ...
  updatedAt: new Date(),
  deletedAt: null,
  isAccessible() { return this.deletedAt === null && this.isActive; },
  ...overrides,
} as UserEntity);
...
class SecurityStateFake implements AuthenticationSecurityRepository {
  ...
  async getState(userUuid: string): Promise<AuthenticationSecurityState> {
    ...
    if (!state) {
      state = { userUuid, failedLoginAttempts: 0, lockedUntil: null, lastLoginAt: null, lastLoginIp: null, updatedAt: new Date() };
      this.states.set(userUuid, state);
    }
    return state;
  }
  async recordFailedLogin(userUuid: string, now: Date, policy: AuthenticationLockoutPolicy): Promise<AuthenticationSecurityState> {
    const state = await this.getState(userUuid);
    state.failedLoginAttempts += 1;
    state.updatedAt = now;
    if (state.failedLoginAttempts >= policy.threshold && (!state.lockedUntil || state.lockedUntil <= now)) {
      state.lockedUntil = new Date(now.getTime() + policy.durationMs);
    }
    return state;
  }
  async recordSuccessfulLogin(userUuid: string, now: Date, context: { ipAddress?: string }): Promise<void> {
    const state = await this.getState(userUuid);
...
const makeLogin = (passwordValid = true, account = user()) => {
  ...
  const credentials = {
    findByUserUuid: vi.fn().mockResolvedValue({ userUuid: account.uuid, passwordHash: 'argon2-hash' }),
  } as unknown as CredentialRepository;
  const security = new SecurityStateFake();
  const sessions = { create: vi.fn(), isActive: vi.fn().mockResolvedValue(true), revoke: vi.fn() } as unknown as AuthenticationSessionRepository;
  const auditEvents: SecurityAuditEvent[] = [];
  const audit: SecurityAuditRepository = { record: vi.fn(async event => { auditEvents.push(event); }) };
  const hasher = { verify: vi.fn().mockResolvedValue(passwordValid) };
  const jwt = { issueAccessToken: vi.fn().mockResolvedValue('signed.jwt.token'), getExpiresAt: vi.fn().mockReturnValue(new Date(Date.now() + 60000)) };
  const service = new LoginService(users, credentials, security, sessions, audit, hasher as never, jwt as never, config as never);
...
describe('authentication security steps 102-106', () => {
  it('102: authenticates successfully, creates session, resets failures, audits safely, and returns no credential data', async () => {
    ...
    const result = await ctx.service.execute({ identifier: 'member@example.com', password: 'CorrectPassword!', ipAddress: '127.0.0.1' });
  ...
  it('103: rejects invalid password and unknown identity with the same application failure result', async () => {
    ...
    await expect(invalid.service.execute({ identifier: 'member@example.com', password: 'wrong' })).resolves.toBeNull();
    ...
    await expect(unknown.service.execute({ identifier: 'missing@example.com', password: 'wrong' })).resolves.toBeNull();
  ...
  it('103: rejects disabled accounts before token issuance', async () => {
    ...
    await expect(ctx.service.execute({ identifier: 'member@example.com', password: 'CorrectPassword!' })).resolves.toBeNull();
  ...
  it('106: locks after threshold, rejects locked attempts, and recovers on successful authentication after expiry', async () => {
    ...
    for (let i = 0; i < 5; i += 1) {
      await ctx.service.execute({ identifier: 'member@example.com', password: 'wrong' });
    }
  ...
  it('106: repeated concurrent failures leave the security state locked consistently', async () => {
    const ctx = makeLogin(false);
    await Promise.all(Array.from({ length: 10 }, () => ctx.service.execute({ identifier: 'member@example.com', password: 'wrong' })));
    const state = await ctx.security.getState('u-1');
...
describe('jwt security steps 91-99 and 104-105', () => {
  it('issues minimal expiring claims and rejects expired/malformed/tampered tokens', async () => {
    ...
    await expect(service.verifyAccessToken('not-a-jwt')).rejects.toThrow('Invalid authentication token');
    const parts = token.split('.');
    parts[1] = Buffer.from(JSON.stringify({ ...JSON.parse(Buffer.from(parts[1], 'base64url').toString()), sub: 'u-2' })).toString('base64url');
    await expect(service.verifyAccessToken(parts.join('.'))).rejects.toThrow('Invalid authentication token');
  });

  it('rejects missing subject, invalid issuer and invalid audience', async () => {
    const jwt = new JwtService({});
    const base = { secret, algorithm: 'HS256' as const, expiresIn: '60s', issuer: 'estate-pro-api', audience: 'estate-pro-client' };
    const missingSub = await jwt.signAsync({ sid: 's-1' }, base);
    await expect(new JwtTokenService(jwt, config as never).verifyAccessToken(missingSub)).rejects.toThrow();
    const wrongIssuer = await jwt.signAsync({ sub: 'u-1', sid: 's-1' }, { ...base, issuer: 'other' });
    await expect(new JwtTokenService(jwt, config as never).verifyAccessToken(wrongIssuer)).rejects.toThrow();
    const wrongAudience = await jwt.signAsync({ sub: 'u-1', sid: 's-1' }, { ...base, audience: 'other' });
    await expect(new JwtTokenService(jwt, config as never).verifyAccessToken(wrongAudience)).rejects.toThrow();
  });
});
