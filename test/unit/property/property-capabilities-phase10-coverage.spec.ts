...
const actor = uuid;
const amenity = { uuid, code: 'POOL', name: 'Pool', category: 'OUTDOOR' as const, description: null, isActive: true, sortOrder: 1 };
const document = { uuid, propertyUuid: uuid, classification: 'LEGAL' as const, title: 'Deed', visibility: 'PRIVATE' as const, status: 'ACTIVE' as const, currentVersion: 1, retentionUntil: null, versions: [] };

describe('PropertyCapabilitiesService phase 10 coverage', () => {
  const makeRepo = (): PropertyCapabilitiesRepository => ({
    listAmenities: vi.fn().mockResolvedValue([amenity]),
    getAmenity: vi.fn().mockResolvedValue(amenity),
    createAmenity: vi.fn().mockResolvedValue(amenity),
    ...
    assignAmenity: vi.fn().mockResolvedValue({ amenity, available: true, value: null, notes: null }),
    ...
    createDocumentVersion: vi.fn().mockResolvedValue({ currentVersion: 2 }),
    updateDocument: vi.fn().mockResolvedValue(document),
    deleteDocument: vi.fn().mockResolvedValue(undefined),
    recordHistory: vi.fn().mockResolvedValue({ uuid: 'history-1' }),
  ...
  it('covers amenity and assignment operations and validation', async () => {
    ...
    await service.updateAmenity(uuid, { code: ' new-code ', name: ' New Name ', description: ' Desc ' }, actor);
    await service.deleteAmenity(uuid, actor);
    await service.listPropertyAmenities(uuid, true);
    await service.assignAmenity(uuid, uuid, { available: true, value: 'A', notes: 'N' }, actor);
    await service.unassignAmenity(uuid, uuid, actor);
    expect(repository.updateAmenity).toHaveBeenCalledWith(uuid, expect.objectContaining({ code: 'NEW-CODE', name: 'New Name', description: 'Desc' }));
    await expect(service.createAmenity({ code: 'POOL', name: 'P', category: 'OUTDOOR' }, actor)).rejects.toBeInstanceOf(BadRequestException);
    await expect(service.updateAmenity(uuid, { name: 'x'.repeat(121) }, actor)).rejects.toBeInstanceOf(BadRequestException);
    await expect(service.assignAmenity(uuid, uuid, { value: 'x'.repeat(121) }, actor)).rejects.toBeInstanceOf(BadRequestException);
    await expect(service.assignAmenity(uuid, uuid, { notes: 'x'.repeat(501) }, actor)).rejects.toBeInstanceOf(BadRequestException);
  });
  ...
  it('covers document lifecycle, history pagination and error mapping', async () => {
    ...
    await expect(service.getDocument(uuid, uuid)).rejects.toBeInstanceOf(NotFoundException);
    await service.createDocument(uuid, {
      classification: 'LEGAL', title: ' Deed ', visibility: 'PRIVATE', status: 'ACTIVE',
      storageKey: 'docs/deed.pdf', mimeType: 'application/pdf', checksumSha256: 'A'.repeat(64), fileSizeBytes: 10,
    }, actor);
    await service.createDocumentVersion(uuid, uuid, { storageKey: 'docs/v2.pdf', mimeType: 'application/pdf', checksumSha256: 'B'.repeat(64), fileSizeBytes: 20 }, actor);
    await service.updateDocument(uuid, uuid, { title: ' Updated ' }, actor);
    await service.deleteDocument(uuid, uuid, actor);
    await service.createHistory(uuid, { event: 'UPDATED', summary: '  Changed  ' }, actor);
    await service.listHistory(uuid, 0.2, 500);
    expect(repository.listHistory).toHaveBeenCalledWith(uuid, 1, 100, undefined);
    await expect(service.createHistory(uuid, { event: 'BAD' as never, summary: 'x' }, actor)).rejects.toBeInstanceOf(BadRequestException);
    await expect(service.createHistory(uuid, { event: 'UPDATED', summary: ' '.repeat(2) }, actor)).rejects.toBeInstanceOf(BadRequestException);
