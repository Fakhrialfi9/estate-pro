...
import { PropertyTypeAlreadyExistsException, PropertyTypeNotFoundException } from '../../../src/modules/property/domain/errors/property-type.errors.js';
import type { PropertyTypeRepository, PropertyTypeListQuery } from '../../../src/modules/property/domain/repositories/property-type.repository.js';
...
const snapshot = (overrides: Partial<Parameters<typeof PropertyTypeEntity.create>[0]> = {}) => ({
  uuid: randomUUID(), code: 'HOUSE', name: 'House', slug: 'house', description: null, icon: null, isActive: true, sortOrder: 0,
  createdAt: new Date('2026-08-27T00:00:00.000Z'), updatedAt: new Date('2026-08-27T00:00:00.000Z'), deletedAt: null, ...overrides,
});
const dto = (overrides: Partial<CreatePropertyTypeDto> = {}): CreatePropertyTypeDto => Object.assign(new (class {})(), { code: 'house', name: '  House  ', slug: 'House', ...overrides }) as CreatePropertyTypeDto;
const updateDto = (overrides: Partial<UpdatePropertyTypeDto> = {}): UpdatePropertyTypeDto => Object.assign(new (class {})(), overrides) as UpdatePropertyTypeDto;
const repository = (): PropertyTypeRepository => ({ create: vi.fn(), findById: vi.fn(), findByCode: vi.fn(), findBySlug: vi.fn(), list: vi.fn(), update: vi.fn(), softDelete: vi.fn() });
...
describe('PropertyType domain and application', () => {
  it('enforces domain invariants and soft-delete state', () => {
    expect(() => PropertyTypeEntity.create(snapshot({ code: 'house' }))).toThrow('Invalid property type code');
    expect(() => PropertyTypeEntity.create(snapshot({ sortOrder: -1 }))).toThrow('Invalid property type sort order');
  ...
  it('creates normalized property types and audits the mutation', async () => {
    const repo = repository(); const audits = audit();
    const created = PropertyTypeEntity.create(snapshot({ code: 'HOUSE', slug: 'house' }));
    vi.mocked(repo.findByCode).mockResolvedValue(null); vi.mocked(repo.findBySlug).mockResolvedValue(null); vi.mocked(repo.create).mockResolvedValue(created);
    const result = await new CreatePropertyTypeUseCase(repo, audits).execute(dto(), context);
    expect(result).toBe(created);
    expect(vi.mocked(repo.create).mock.calls).toContainEqual([expect.objectContaining({ code: 'HOUSE', name: 'House', slug: 'house' })]);
    expect(vi.mocked(audits.record).mock.calls).toContainEqual([expect.objectContaining({ action: 'PROPERTY_TYPE_CREATED' })]);
  });

  it('rejects duplicate code and duplicate slug on create', async () => {
    const repo = repository(); const existing = PropertyTypeEntity.create(snapshot());
    ...
    const slugRepo = repository(); vi.mocked(slugRepo.findByCode).mockResolvedValue(null); vi.mocked(slugRepo.findBySlug).mockResolvedValue(existing);
  ...
  it('updates allowed fields and rejects duplicate identifiers', async () => {
    const repo = repository(); const audits = audit();
    const current = PropertyTypeEntity.create(snapshot()); const updated = PropertyTypeEntity.create(snapshot({ code: 'VILLA', name: 'Villa', slug: 'villa' }));
    vi.mocked(repo.findById).mockResolvedValue(current); vi.mocked(repo.findByCode).mockResolvedValue(null); vi.mocked(repo.findBySlug).mockResolvedValue(null); vi.mocked(repo.update).mockResolvedValue(updated);
    ...
    expect(vi.mocked(repo.update).mock.calls).toContainEqual([current.uuid, expect.objectContaining({ code: 'VILLA', name: 'Villa', slug: 'villa' })]);
  ...
  it('returns current entity for an empty update and rejects missing entities', async () => {
    const repo = repository(); const current = PropertyTypeEntity.create(snapshot()); vi.mocked(repo.findById).mockResolvedValue(current);
  ...
  it('lists with validated pagination, filter, and sorting', async () => {
    ...
    const query: PropertyTypeListQuery = { page: 2, limit: 20, filterField: 'isActive', filterValue: true, sortBy: 'sortOrder', sortDirection: 'asc', search: 'house' };
    ...
    await expect(new ListPropertyTypesUseCase(repo).execute({ ...query, filterValue: 'true' as never })).rejects.toThrow('isActive filterValue');
  });

  it('soft-deletes existing property types', async () => {
    const repo = repository(); const audits = audit(); const current = PropertyTypeEntity.create(snapshot());
    vi.mocked(repo.findById).mockResolvedValue(current); vi.mocked(repo.softDelete).mockResolvedValue(undefined);
    await expect(new DeletePropertyTypeUseCase(repo, audits).execute(current.uuid, context)).resolves.toBeUndefined();
    expect(vi.mocked(repo.softDelete).mock.calls).toContainEqual([current.uuid]);
    expect(vi.mocked(audits.record).mock.calls).toContainEqual([expect.objectContaining({ action: 'PROPERTY_TYPE_DELETED' })]);
  });
});
