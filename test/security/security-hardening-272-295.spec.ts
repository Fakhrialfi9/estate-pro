...
const loginHarness = (validPassword: boolean) => {
  ...
  const hasher = {
    verify: vi.fn().mockResolvedValue(validPassword),
...
describe('STEP 273', () => {
  it('enforces a finite login throttle for repeated credential attempts', async () => {
    const h = loginHarness(false);
    await Promise.all(
      ['a@example.com', 'b@example.com', 'c@example.com'].map((identifier) =>
        h.service.execute({ identifier, password: 'wrong' }),
      ),
    );
    ...
    expect(
      Reflect.getMetadataKeys(AuthController.prototype.loginUser).some((key) =>
...
describe('STEPS 276–279', () => {
  ...
  it('276 rejects algorithm confusion and alg=none', async () => {
    ...
    await expect(service().verifyAccessToken(confused)).rejects.toThrow(
      UnauthorizedException,
    );
    const payload = confused.split('.')[1];
    const none = `${Buffer.from(JSON.stringify({ alg: 'none', typ: 'JWT' })).toString('base64url')}.${payload}.`;
    await expect(service().verifyAccessToken(none)).rejects.toThrow(
      UnauthorizedException,
    );
  });
  ...
  it('278 rejects invalid JWT signatures', async () => {
    ...
    const token = await new JwtService({ secret: wrong }).signAsync(
      { sub: UUID_A, sid: 's1' },
      {
        secret: wrong,
        algorithm: 'HS256',
...
describe('STEPS 280–282', () => {
  it('280 uses a fresh server-side session secret instead of a client supplied identifier', async () => {
    ...
    expect(
      (h.sessions.create.mock.calls[0]?.[1] as { sessionId: string }).sessionId,
    ).not.toBe('attacker-fixed-session');
  });
  ...
  it('282 checks expiry and revocation in the actual Prisma session repository', async () => {
    ...
    await expect(repo.isActive(UUID_A, 'secret', new Date())).resolves.toBe(
      false,
    );
...
describe('STEPS 283–286', () => {
  it('283 denies IDOR across user profiles', () => {
    ...
    expect(() =>
      policy.assertCanManage({ sub: UUID_A, permissions: [] }, UUID_B),
    ).toThrow();
    expect(() =>
      policy.assertCanManage(
        { sub: UUID_A, permissions: ['users:manage'] },
        UUID_B,
      ),
    ).not.toThrow();
  });
  ...
  it('285 blocks protected role escalation without explicit protected-role permission', () => {
    ...
    expect(() =>
      policy.canManage(
        { userUuid: UUID_A, permissions: ['roles:manage'] },
        true,
      ),
    ).toThrow(ForbiddenException);
    expect(() =>
  ...
  it('286 rejects permission escalation using spoofed request permissions', async () => {
    ...
    const request = {
      user: { sub: UUID_A, permissions: ['permissions:manage'] },
    };
...
describe('STEP 288', () => {
  it('uses one-time recovery-code persistence semantics', async () => {
    ...
    await recoveryDelegate.updateMany({
      where: { id: 1n, usedAt: null },
...
describe('STEP 289', () => {
  it('passes SQL injection payloads as Prisma values', async () => {
    ...
    expect(findFirst).toHaveBeenCalledWith({
      where: { email: "' OR 1=1 --", deletedAt: null },
    });
  });
});
