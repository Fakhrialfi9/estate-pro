...
describe('Permission authorization security', () => {
  ...
  it('denies regular users permission management with a semantic error code', () => {
    expect(() => policy.canManage(actor([]))).toThrow(
      ForbiddenPermissionOperationException,
    );

    try {
      policy.canManage(actor([]));
    } catch (error: unknown) {
      expect(error).toBeInstanceOf(ForbiddenPermissionOperationException);
      expect((error as ForbiddenPermissionOperationException).code).toBe(
