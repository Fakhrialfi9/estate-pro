...
async function createUser(): Promise<{ uuid: string; email: string }> {
  ...
  const user = await prisma.authenticationUser.create({
    data: { uuid: randomUUID(), email, username: `dynamic-${randomUUID().slice(0, 12)}`, status: 'active', isActive: true, isVerified: true },
  });
  await prisma.authenticationUserCredential.create({ data: { userId: user.id, passwordHash: await hasher.hash(PASSWORD) } });
...
describe('Dynamic HTTP security baseline — STEPS 227–260', () => {
  ...
  it('blocks unauthenticated access and rejects injection/mass-assignment payloads at the HTTP boundary', async () => {
    ...
    const injection = await http().post(`${AUTH}/login`).send({ identifier: "admin' OR '1'='1", password: '<script>alert(1)</script>' });
    expect(injection.status).toBe(401);
    expect(JSON.stringify(injection.body)).not.toContain("admin' OR '1'='1");
    expect(JSON.stringify(injection.body)).not.toContain('<script>alert(1)</script>');
    await http().post(`${AUTH}/login`).send({ identifier: 'nobody@example.test', password: PASSWORD, role: 'admin', permissions: ['*'] }).expect(400);
  });

  it('performs real refresh rotation, replay detection, and logout invalidation over HTTP', async () => {
    const user = await createUser();
    const login = await http().post(`${AUTH}/login`).send({ identifier: user.email, password: PASSWORD }).expect(201);
    const first = login.body as AuthResponse;
    const rotated = await http().post(`${AUTH}/refresh`).send({ refreshToken: first.refreshToken }).expect(201);
    ...
    await http().post(`${AUTH}/logout`).set('Authorization', `Bearer ${second.accessToken}`).expect(201);
    await http().get(`${AUTH}/me`).set('Authorization', `Bearer ${second.accessToken}`).expect(401);
  ...
  it('does not use an ambient cookie credential for refresh and keeps the refresh response non-cacheable', async () => {
    ...
    const response = await http().post(`${AUTH}/refresh`).set('Origin', 'https://attacker.example').send({ refreshToken: auth.refreshToken }).expect(201);
