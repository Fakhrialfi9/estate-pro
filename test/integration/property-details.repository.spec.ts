...
async function createProperty() {
  ...
  return prisma.property.create({ data: { uuid: randomUUID(), businessCode: `B-${randomUUID().replaceAll('-', '').slice(0, 24)}`, referenceNumber: `R-${randomUUID()}`, propertyTypeId: type.id, propertyCategoryId: category.id, title: 'Integration Property', slug: `property-${randomUUID()}` } });
}

async function createFacility(active = true) {
  return prisma.facility.create({ data: { uuid: randomUUID(), code: `FAC-${randomUUID().slice(0, 10)}`, name: 'Swimming Pool', slug: `facility-${randomUUID()}`, category: 'RECREATION', isActive: active } });
}
...
describe('Property details repository integration', () => {
  ...
  it('upserts specification values and preserves decimal precision', async () => {
    ...
    const saved = await repository.upsertSpecifications(property.uuid, { landArea: '12345.67', buildingArea: '4321.12', bedrooms: 4, bathrooms: '3.50', floors: 2, parkingType: 'GARAGE', parkingSpaces: 2, yearBuilt: 2020, yearRenovated: 2024 }, actor);
