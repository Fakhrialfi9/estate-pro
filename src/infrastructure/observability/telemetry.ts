  ...
  try {
    ...
    const message =
      error instanceof Error
        ? error.message
        : 'Unknown telemetry startup error.';
    process.stderr.write(
      JSON.stringify({
        ...
        message:
          'OpenTelemetry startup failed; application will continue without telemetry.',
        error: {
...
export const shutdownTelemetry = async (): Promise<void> => {
  ...
  try {
    ...
    const message =
      error instanceof Error
        ? error.message
        : 'Unknown telemetry shutdown error.';
    process.stderr.write(
      JSON.stringify({
        level: 'error',
        timestamp: new Date().toISOString(),
        ...
        environment: metadata.environment,
        message:
          'OpenTelemetry shutdown failed; application shutdown will continue.',
        error: {
          type: error instanceof Error ? error.name : 'UnknownError',
          message,
        },
