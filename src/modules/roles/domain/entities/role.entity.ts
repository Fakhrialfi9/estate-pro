...
export class RoleEntity {
  ...
  get isSystem(): boolean { return this.snapshot.isSystem; }

  update(changes: RoleUpdate): void {
    if (changes.name !== undefined) this.snapshot.name = normalizeRoleName(changes.name);
    if (changes.description !== undefined) {
      this.snapshot.description = changes.description === null
        ? null
  ...
  toSnapshot(): RoleSnapshot { return { ...this.snapshot }; }
  ...
  private static validateSnapshot(snapshot: RoleSnapshot): void {
    ...
    if (!snapshot.name || snapshot.name.length > MAX_NAME_LENGTH) throw new Error('Invalid role name');
    if (!snapshot.code || snapshot.code.length > MAX_CODE_LENGTH || !CODE_PATTERN.test(snapshot.code)) {
