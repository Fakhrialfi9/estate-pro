...
interface RoleClaims { sub: string; permissions?: string[]; }
...
abstract class BaseRoleAccessGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    if (!(request.user.permissions ?? []).includes(this.requiredPermission)) throw new ForbiddenException();
    return true;
  }
}
...
export class RoleReadAccessGuard extends BaseRoleAccessGuard {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    if (!permissions.has(ROLE_READ_PERMISSION) && !permissions.has(ROLE_MANAGE_PERMISSION)) throw new ForbiddenException();
    return true;
  }
}
