...
const lineTotal = (quantity: number, unitAmount: string): string => {
  ...
  return formatAmount(scaledAmount(unitAmount) * BigInt(quantity));
};

type Tx = Pick<PrismaService, 'salesDealItem' | 'salesDeal'>;
...
export class PrismaSalesRepository implements SalesRepository {
  ...
  async reorderStages(pipelineUuid: string, orderedStageUuids: string[]) {
    ...
    if (
      ...
      orderedStageUuids.some(
        (uuid) => !current.some((stage) => stage.uuid === uuid),
      )
    )
      throw new ConflictException(
        'Reorder set must exactly match pipeline stages',
      );
  ...
  async createOpportunity(input: OpportunityInput) {
    try {
      ...
      if (isUniqueViolation(error)) {
        ...
        throw new ConflictException(
          'Opportunity already exists for this command',
        );
      }
      throw error;
    }
  ...
  async findConversion(leadUuid: string) {
    const record = await this.prisma.salesOpportunity.findUnique({
      where: { leadUuid },
    });
  ...
  async assignOpportunity(
    ...
    return toOpportunity(
      await this.prisma.salesOpportunity.update({
        where: { uuid },
        data: { ownerUserUuid, teamUuid, version: { increment: 1 } },
      }),
    );
  }
  async transitionOpportunity(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (current.status !== from)
        throw new ConflictException('Opportunity changed concurrently');
      const updated = await tx.salesOpportunity.updateMany({
        where: { uuid, status: from, version: current.version },
        data: {
          status: to,
          version: { increment: 1 },
          ...(to === 'LOST' && reason ? { lostReasonUuid: reason } : {}),
        },
      });
      if (updated.count !== 1)
        throw new ConflictException('Opportunity changed concurrently');
  ...
  async updateActivityStatus(uuid: string, status: ActivityStatus) {
    const current = await this.prisma.salesActivity.findUnique({
      where: { uuid },
    });
  ...
  async createViewing(input: ViewingInput, actor: SalesActor) {
    ...
    if (conflict)
      throw new ConflictException(
        'Property already has a viewing at this time',
      );
  ...
  async updateViewingStatus(
    ...
    return this.prisma.$transaction(async (tx) => {
      const current = await tx.salesViewing.findUnique({ where: { uuid } });
      ...
      const result = await tx.salesViewing.updateMany({
        where: { uuid, status: current.status },
        data: { status, ...(scheduledAt ? { scheduledAt } : {}) },
      });
      if (result.count !== 1)
        throw new ConflictException('Viewing changed concurrently');
      return tx.salesViewing.findUnique({ where: { uuid } });
    });
  }
  ...
  async transitionNegotiation(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (!negotiationTransitionAllowed(current.status, status))
        throw new ConflictException(
          'Negotiation lifecycle transition is invalid',
        );
      ...
      if (updated.count !== 1)
        throw new ConflictException('Negotiation changed concurrently');
  ...
  async createOffer(input: OfferInput) {
    ...
    try {
      return await this.prisma.salesOffer.create({
        data: {
          uuid: randomUUID(),
      ...
      if (isUniqueViolation(error))
        throw new ConflictException(
          'Offer version changed concurrently; retry',
        );
      throw error;
    }
  }
  async transitionOffer(uuid: string, status: OfferStatus) {
    return this.prisma.$transaction(async (tx) => {
      ...
      if (updated.count !== 1)
        throw new ConflictException('Offer changed concurrently');
  ...
  async createDeal(input: DealInput) {
    return this.prisma.$transaction(async (tx) => {
      ...
      if (!['QUALIFIED', 'NEGOTIATING'].includes(opportunity.status))
        throw new ConflictException(
          'Opportunity is not eligible for deal conversion',
        );
      if (!input.offerUuid)
        throw new ConflictException('Accepted offer is required');
      const offer = await tx.salesOffer.findUnique({
        where: { uuid: input.offerUuid },
      });
      if (!offer || offer.status !== 'ACCEPTED')
        throw new ConflictException('Accepted offer is required');
      const negotiation = await tx.salesNegotiation.findUnique({
        where: { uuid: offer.negotiationUuid },
      });
  ...
  private async recalculateDealTotal<T extends Tx>(dealUuid: string, tx: T) {
    ...
    await tx.salesDeal.update({
      where: { uuid: dealUuid },
      data: { totalAmount: formatAmount(total) },
    });
  }
  async addDealItem(input: DealItemInput) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      await this.recalculateDealTotal(input.dealUuid, tx);
      return item;
    });
  }
  async updateDealItem(uuid: string, input: Partial<DealItemInput>) {
    const current = await this.prisma.salesDealItem.findUnique({
      where: { uuid },
    });
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      await this.recalculateDealTotal(current.dealUuid, tx);
      return item;
    });
  }
  ...
  async closeDeal(
    ...
    return this.prisma.$transaction(async (tx) => {
      const existingByKey = await tx.salesClosing.findUnique({
  ...
  async markLost(
    ...
    const reason = await this.prisma.salesLostReason.findUnique({
      where: { uuid: reasonUuid },
    });
    if (!reason || !reason.isActive)
      throw new ConflictException('Active lost reason is required');
    if (targetType === 'OPPORTUNITY') {
      const current = await this.getOpportunity(uuid);
      if (!current) throw new NotFoundException('Opportunity not found');
      if (!['OPEN', 'QUALIFIED', 'NEGOTIATING'].includes(current.status))
        throw new ConflictException(
          'Opportunity cannot be marked lost from current state',
        );
      return this.prisma.$transaction(async (tx) => {
        const updated = await tx.salesOpportunity.updateMany({
          where: { uuid, status: current.status, version: current.version },
        ...
        const result = await tx.salesOpportunity.findUnique({
          where: { uuid },
        });
        if (!result) throw new NotFoundException('Opportunity not found');
        return toOpportunity(result);
    ...
    const current = await this.getDeal(uuid);
    if (!current) throw new NotFoundException('Deal not found');
    if (['CLOSED', 'LOST', 'CANCELLED'].includes(String(current.status)))
      throw new ConflictException('Deal is terminal');
  ...
  async listLostReasons() {
    return this.prisma.salesLostReason.findMany({
      where: { isActive: true },
      orderBy: { name: 'asc' },
    });
  }
  async createLostReason(input: {
    code: string;
    name: string;
    isActive?: boolean;
  }) {
    try {
      return await this.prisma.salesLostReason.create({
        data: {
          uuid: randomUUID(),
          code: input.code.trim().toUpperCase(),
          name: input.name.trim(),
      ...
      if (isUniqueViolation(error))
        throw new ConflictException('Lost reason code already exists');
      throw error;
    }
  }
  ...
  async calculateCommission(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (!deal || deal.status !== 'CLOSED')
        throw new ConflictException('Closed deal is required');
      const rule = await tx.salesCommissionRule.findUnique({
        where: { uuid: ruleUuid },
      });
      if (!rule || !rule.isActive)
        throw new ConflictException('Active commission rule is required');
      const amount = formatAmount(
        (scaledAmount(String(deal.totalAmount ?? '0')) *
          scaledAmount(String(rule.ratePercent))) /
          1000000n,
      );
  ...
  async approveCommission(uuid: string) {
    const current = await this.prisma.salesCommission.findUnique({
      where: { uuid },
    });
    if (!current) throw new NotFoundException('Commission not found');
    if (current.status !== 'PENDING')
      throw new ConflictException('Commission must be pending');
  ...
  async settleCommission(uuid: string) {
    ...
    if (current.status !== 'APPROVED')
      throw new ConflictException('Commission must be approved');
    return this.prisma.salesCommission.update({
      where: { uuid },
  ...
  async commissionReport(query: Record<string, unknown>) {
    ...
    const [items, total] = await Promise.all([
      this.prisma.salesCommission.findMany({
        ...
        skip: (page - 1) * limit,
        take: limit,
        orderBy: [{ calculatedAt: 'desc' }, { uuid: 'desc' }],
      }),
      this.prisma.salesCommission.count({ where }),
    ]);
    const all = await this.prisma.salesCommission.findMany({
      where,
      select: { amount: true },
    });
  ...
  async forecast(query: Record<string, unknown>) {
    ...
    return {
      ...
      pipelineValue: formatAmount(pipeline),
      weightedForecast: formatAmount(weighted),
