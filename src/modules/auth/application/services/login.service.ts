...
export interface AccessTokenLoginResponse { accessToken: string; tokenType: 'Bearer'; expiresIn: number; }
...
export class LoginService {
  ...
  async execute(command: LoginCommand): Promise<LoginResponse | null> {
    ...
    if (!user) {
      await this.auditFailure(command, AUTH_FAILURE_REASONS.INVALID_CREDENTIALS);
      return null;
    }
    const state = await this.security.getState(user.uuid);
    if (this.isLocked(state.lockedUntil, now)) {
      await this.auditFailure(command, AUTH_FAILURE_REASONS.ACCOUNT_LOCKED, user.uuid);
      return null;
    }
    if (!user.isAccessible() || user.status !== 'active') {
      await this.auditFailure(command, AUTH_FAILURE_REASONS.ACCOUNT_DISABLED, user.uuid);
      return null;
    }
    ...
    if (!validPassword) {
      ...
      await this.auditFailure(command, AUTH_FAILURE_REASONS.INVALID_CREDENTIALS, user.uuid);
      return null;
    }
  ...
  async executeMfa(input: { challengeToken: string; code?: string; recoveryCode?: string; ipAddress?: string; userAgent?: string; requestId?: string }): Promise<AccessTokenLoginResponse | null> {
    const userUuid = await this.twoFactor.verifyLoginChallenge({ token: input.challengeToken, code: input.code, recoveryCode: input.recoveryCode, context: { ipAddress: input.ipAddress, userAgent: input.userAgent, requestId: input.requestId } });
  ...
  private async issueSession(userUuid: string, command: LoginCommand | { ipAddress?: string; userAgent?: string; requestId?: string }, now: Date): Promise<AccessTokenLoginResponse> {
    ...
    await this.sessions.create(userUuid, { sessionId, ipAddress: command.ipAddress, userAgent: command.userAgent, expiresAt, requestId: command.requestId });
  ...
  private async auditFailure(command: LoginCommand, reason: string, subjectUuid?: string): Promise<void> {
    await this.audit.record({ action: AUTH_ACTIONS.LOGIN_FAILURE, subjectUuid, entityType: 'authentication', result: 'FAILURE', reason, ipAddress: command.ipAddress, userAgent: command.userAgent, requestId: command.requestId });
  }
