...
import type { AuthenticationSessionCreation, AuthenticationSessionRepository, SessionAuditContext, SessionListQuery } from '../../domain/repositories/authentication-session.repository.js';
...
export const SESSION_AUDIT_ACTIONS = {
  CREATED: 'SESSION_CREATED', REVOKED: 'SESSION_REVOKED', LOGOUT: 'LOGOUT', LOGOUT_ALL: 'LOGOUT_ALL_SESSIONS',
  ADMIN_REVOKED: 'SESSION_ADMIN_REVOKED', PASSWORD_CHANGE_REVOKED: 'PASSWORD_CHANGE_SESSION_REVOKED', SECURITY_EVENT_REVOKED: 'SECURITY_EVENT_SESSION_REVOKED',
} as const;
const MAX_PAGE_SIZE = 100;
const MAX_USER_AGENT_LENGTH = 1024;
const SESSION_SECRET_BYTES = 32;
export interface CreateSessionInput extends SessionAuditContext { sessionId: string; ipAddress?: string; userAgent?: string; expiresAt: Date; }

@Injectable()
export class SessionService implements SessionSecurityPort {
  constructor(@Inject(AUTHENTICATION_SESSION_REPOSITORY) private readonly sessions: AuthenticationSessionRepository, @Inject(SECURITY_AUDIT_REPOSITORY) private readonly audit: SecurityAuditRepository) {}
  ...
  async create(userUuid: string, input: CreateSessionInput): Promise<SessionEntity> {
    ...
    const snapshot = await this.sessions.create(userUuid, { sessionId: input.sessionId, ipAddress: input.ipAddress, userAgent: this.normalizeUserAgent(input.userAgent), expiresAt: input.expiresAt });
  ...
  isActive(userUuid: string, sessionId: string, now = new Date()): Promise<boolean> { return this.sessions.isActive(userUuid, sessionId, now); }
  ...
  async logoutCurrent(userUuid: string, sessionId: string, context: SessionAuditContext = {}): Promise<void> {
    ...
    await this.audit.record({ action: SESSION_AUDIT_ACTIONS.LOGOUT, actorUuid: userUuid, subjectUuid: userUuid, entityType: 'session', result: 'SUCCESS', ...context });
  }
  async revokeOwnSession(userUuid: string, publicSessionId: string, context: SessionAuditContext = {}): Promise<void> {
    ...
    await this.audit.record({ action: SESSION_AUDIT_ACTIONS.REVOKED, actorUuid: userUuid, subjectUuid: userUuid, entityType: 'session', result: 'SUCCESS', ...context });
  }
  async logoutAll(userUuid: string, context: SessionAuditContext = {}): Promise<number> {
    ...
    await this.audit.record({ action: SESSION_AUDIT_ACTIONS.LOGOUT_ALL, actorUuid: userUuid, subjectUuid: userUuid, entityType: 'session', result: 'SUCCESS', ...context });
    return count;
  }
  async adminRevoke(actorUserUuid: string, targetUserUuid: string, publicSessionId: string, context: SessionAuditContext = {}): Promise<void> {
    ...
    await this.audit.record({ action: SESSION_AUDIT_ACTIONS.ADMIN_REVOKED, actorUuid: actorUserUuid, subjectUuid: targetUserUuid, entityType: 'session', result: 'SUCCESS', ...context });
  }
  async revokeAllForSecurityEvent(userUuid: string, event: Parameters<SessionSecurityPort['revokeAllForSecurityEvent']>[1], context: SessionAuditContext = {}): Promise<number> {
    const count = await this.sessions.revokeAll(userUuid, new Date());
    await this.audit.record({ action: event === 'PASSWORD_CHANGE' ? SESSION_AUDIT_ACTIONS.PASSWORD_CHANGE_REVOKED : SESSION_AUDIT_ACTIONS.SECURITY_EVENT_REVOKED, actorUuid: userUuid, subjectUuid: userUuid, entityType: 'session', result: 'SUCCESS', ...context });
    return count;
  }
  private normalizeUserAgent(userAgent?: string): string | undefined { if (!userAgent) return undefined; return userAgent.slice(0, MAX_USER_AGENT_LENGTH); }
}
