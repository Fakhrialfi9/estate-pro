...
export class SessionService implements SessionSecurityPort {
  constructor(
    ...
    @Inject(REFRESH_TOKEN_SECURITY_PORT)
    private readonly refreshTokens: RefreshTokenSecurityPort,
  ) {}
  ...
  async revokeAllForSecurityEvent(
    ...
    const now = new Date();
    const count = await this.sessions.revokeAll(userUuid, now);
    const reason: RefreshTokenRevokeReason =
        ...
        : event === 'PASSWORD_RESET'
          ...
          : event === 'ACCOUNT_DISABLED'
            ...
            : event === 'ACCOUNT_LOCKED'
              ? 'ACCOUNT_LOCKED'
              : event === 'ACCOUNT_COMPROMISE'
                ? 'SECURITY_EVENT'
                : event === 'ADMIN_FORCED_LOGOUT'
                  ? 'ADMIN_REVOKED'
                  : 'SECURITY_EVENT';
