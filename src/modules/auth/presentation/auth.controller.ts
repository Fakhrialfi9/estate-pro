...
export class AuthController {
  ...
  @ApiOperation({
    ...
    description:
      'Validates credentials and, when enabled, returns an MFA challenge before issuing a session-bound access and refresh token.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Rotates the opaque refresh token and returns a new access token and refresh token.',
  })
  async refresh(
    ...
    void _cacheControl;
    try {
      return await this.refreshTokens.refresh(
        dto.refreshToken,
        requestContext(request),
      );
    } catch (error) {
  ...
  @ApiOperation({
    ...
    description:
      'Revokes the authenticated session and its refresh-token family.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Returns the authenticated user through the safe user serializer.',
  })
