...
type UserDelegate = { findUnique(args: unknown): Promise<{ id: bigint } | null> };
...
export class PrismaTwoFactorRecoveryCodeRepository implements TwoFactorRecoveryCodeRepository {
  ...
  async replaceAll(userUuid: string, hashes: readonly string[]): Promise<void> {
    ...
    await this.recoveryCodes.createMany({ data: hashes.map((codeHash) => ({ userId: user.id, codeHash })) });
  }

  async findUnused(userUuid: string): Promise<readonly { id: bigint; codeHash: string }[]> {
    return this.recoveryCodes.findMany({ where: { user: { uuid: userUuid }, usedAt: null }, select: { id: true, codeHash: true } });
  }

  async markUsed(id: bigint, usedAt: Date): Promise<boolean> {
    const result = await this.recoveryCodes.updateMany({ where: { id, usedAt: null }, data: { usedAt } });
