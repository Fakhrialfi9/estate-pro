...
export class PrismaTwoFactorRepository implements TwoFactorRepository {
  ...
  async createPending(input: {
    ...
    await this.twoFactor.updateMany({
      ...
      data: {
        ...
        enrollmentStartedAt: input.startedAt,
  ...
  async recordFailedVerification(input: {
    userUuid: string;
    now: Date;
    threshold: number;
    ...
    if (latest && latest.failedVerificationAttempts >= input.threshold) {
      await this.twoFactor.updateMany({
        where: { user: { uuid: input.userUuid } },
        data: {
  ...
  async recordSuccessfulVerification(input: {
    userUuid: string;
    now: Date;
    timeStep: bigint;
  }): Promise<boolean> {
    const result = await this.twoFactor.updateMany({
      where: {
        ...
        OR: [
          { lastUsedTimeStep: null },
          { lastUsedTimeStep: { lt: input.timeStep } },
        ],
      },
      data: {
        lastUsedAt: input.now,
        lastUsedTimeStep: input.timeStep,
        failedVerificationAttempts: 0,
        lockedUntil: null,
      },
    });
  ...
  private toSnapshot(record: RecordShape): TwoFactorSnapshot {
    return {
      ...
      lastUsedTimeStep: record.lastUsedTimeStep,
