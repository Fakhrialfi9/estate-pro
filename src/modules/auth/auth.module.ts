...
@Global()
@Module({
  imports: [ConfigModule, DatabaseModule, UsersModule, PasswordHashingModule, JwtModule.registerAsync({ imports: [ConfigModule], inject: [ConfigService], useFactory: (config: ConfigService) => ({ secret: config.getOrThrow<string>('auth.jwt.secret'), signOptions: { expiresIn: config.getOrThrow<string>('auth.jwt.expiresIn') as RequiredExpiresIn, issuer: config.getOrThrow<string>('auth.jwt.issuer'), audience: config.getOrThrow<string>('auth.jwt.audience'), algorithm: config.getOrThrow<'HS256' | 'HS384' | 'HS512'>('auth.jwt.algorithm') } }) })],
  controllers: [AuthController, TwoFactorController, SessionController, AdminSessionController],
  providers: [
    LoginService, LogoutService, SessionService, JwtTokenService, TotpService, TwoFactorCryptoService, TwoFactorService,
    JwtAuthGuard, SessionAdminGuard,
