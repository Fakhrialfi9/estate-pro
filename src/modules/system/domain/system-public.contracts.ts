...
export interface SystemSettingResponse {
  ...
  readonly valueType: SystemSettingValueType | string;
...
export interface SystemSettingsContract {
  ...
  update(key: string, rawValue: string, actorUuid: string, expectedVersion?: number): Promise<SystemSettingUpdateResult>;
}
...
export interface SystemNotificationsContract {
  list(userUuid: string, page: number, limit: number, unreadOnly: boolean): Promise<unknown>;
...
export interface SystemJobsContract {
  list(input: { page: number; limit: number; state?: string }, actorUuid: string): Promise<unknown>;
