...
export interface SystemActivityRepository {
  ...
  list(input:{page:number;limit:number;actorUuid?:string;eventType?:string;category?:string;resourceType?:string;resourceUuid?:string}):Promise<{items:readonly SystemActivityRecord[];total:number}>;
}
