...
export class SystemExportService {
  ...
  async execute(request: ExportRequest): Promise<ExportResult> {
    ...
    if (request.from) snapshot.from = request.from.toISOString();
    if (request.to) snapshot.to = request.to.toISOString();
    if (request.category) snapshot.category = request.category;
  ...
  private async processJob(
    ...
    try {
      const generator =
        job.format === 'json' ? this.jsonStream(job) : this.csvStream(job);
  ...
  private async *csvStream(job: SystemExportJobRecord) {
    ...
    while (emitted < limit) {
      ...
      for (const row of result.items) {
        ...
        yield `${headers.map((key) => csvCell(values[key])).join(',')}\n`;
        emitted += 1;
        await this.jobs.update(job.uuid, { processedRows: emitted });
        if (emitted >= limit) break;
      }
  ...
  private exportRow(row: ExportRow) {
    return {
      uuid: row.uuid,
