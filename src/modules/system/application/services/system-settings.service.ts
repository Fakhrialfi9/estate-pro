...
export class SystemSettingsService {
  ...
  async list(page: number, limit: number) {
    ...
    const items = result.items.map((item) => ({ ...item, value: this.deserialize(item.valueType, item.value) }));
  ...
  async get(key: string) {
    ...
    return { key: normalizedKey, scope: 'GLOBAL', scopeKey: 'global', valueType: definition.valueType, value: this.deserialize(definition.valueType, raw), ...(stored ? { version: stored.version, updatedAt: stored.updatedAt } : {}) };
  }
  ...
  async update(key: string, rawValue: string, actorUuid: string, expectedVersion?: number) {
    ...
    let result;
    try {
      result = await this.repository.upsert({ key: normalizedKey, scope: definition.scope, scopeKey: 'global', valueType: definition.valueType, value, mutable: definition.mutable, expectedVersion });
    } catch (error: unknown) {
    ...
    await this.audit.record({ action: 'SYSTEM_SETTING_UPDATED', actorUuid, entityType: 'system_setting', entityUuid: result.uuid, result: 'SUCCESS', reason: `key=${normalizedKey};version=${result.version}`, changes: [{ field: 'value', oldValue: '[REDACTED]', newValue: '[REDACTED]' }] });
    await this.activity.append({ actorUuid, eventType: 'SYSTEM_SETTING_UPDATED', category: 'SETTINGS', resourceType: 'system_setting', resourceUuid: result.uuid, summary: `Updated ${normalizedKey}`, metadata: { key: normalizedKey, version: result.version }, requestId: undefined });
