...
const sanitizeFilename = (value: string): string =>
  Array.from(value)
    .map((character) =>
      character.charCodeAt(0) <= 31 || character === '/' || character === '\\'
        ? '_'
        : character,
    )
    .join('');
...
export class SystemImportService {
  ...
  async execute(
    ...
    try {
      ...
      if (
        ...
        rows.length > MAX_ALL_OR_NOTHING_ROWS
      )
        throw new BadRequestException(
          `ALL_OR_NOTHING is limited to ${MAX_ALL_OR_NOTHING_ROWS} rows`,
        );
      if (format === 'csv' && columnMapping.length)
        this.mapping.validateColumnMapping(
          columnMapping,
          rows[0] ? Object.keys(rows[0]) : [],
        );
      const mappedRows = rows.map((row) =>
        this.mapping.applyFieldMapping(
          this.mapping.applyColumnMapping(row, columnMapping),
          fieldMapping,
        ),
      );
      const prepared = mappedRows.map((row, index) =>
        this.prepareRow(row, actorUuid, idempotencyKey, index + 2),
      );
      ...
      if (!input.preview) {
        if (transactionStrategy === 'ALL_OR_NOTHING') {
          const needsUpsert =
            ...
            writes.some((item) => item.existing);
          if (needsUpsert)
            await this.activity.upsertBatch(writes.map((item) => item.data));
          ...
          for (const write of writes) {
            try {
              if (
                ...
                await this.activity.upsert(write.data);
          ...
          for (let offset = 0; offset < writes.length; offset += BATCH_SIZE) {
            ...
            try {
              const needsUpsert =
                conflictStrategy === 'UPDATE' ||
                conflictStrategy === 'UPSERT' ||
                batch.some((item) => item.existing);
              if (needsUpsert)
                await this.activity.upsertBatch(batch.map((item) => item.data));
              else
                await this.activity.appendBatch(batch.map((item) => item.data));
      ...
      return await this.finish(job, actorUuid, {
        state: errors.length ? 'FAILED' : 'SUCCEEDED',
        totalRows: rows.length,
      ...
      return await this.finish(job, actorUuid, {
        ...
        errors: [{ row: 0, message: message.slice(0, 500) }],
      });
    }
  }
    ...
    const resolved =
      ...
      (filename.toLowerCase().endsWith('.json')
        ...
        : filename.toLowerCase().endsWith('.csv')
          ? 'csv'
          : undefined);
    if (!filename || filename.length > 255 || !resolved)
  ...
  private prepareRow(
    ...
    const uuid = createHash('sha256')
      .update(`${idempotencyKey}:${rowNumber}:${JSON.stringify(row)}`)
      .digest('hex')
  ...
  private async finish(
    job: SystemImportJobRecord,
    actorUuid: string,
    input: Pick<
      SystemImportJobRecord,
      'state' | 'totalRows' | 'processedRows' | 'failedRows' | 'errors'
    >,
  ): Promise<ImportResult> {
