...
export class SystemRoadmapControlService {
  ...
  listFlags(environment?: string) {
    return this.roadmap.featureFlag.list(environment);
  }
  ...
  async setFlag(
    ...
    if (!/^[a-z0-9][a-z0-9_-]{0,31}$/i.test(input.environment))
      throw new Error('Invalid environment identifier');
    if (!/^[A-Za-z0-9][A-Za-z0-9._-]{0,159}$/.test(input.key))
    ...
    const row = await this.roadmap.featureFlag.upsert({
      ...
      rolloutPercentage: clamp(input.rolloutPercentage ?? 100, 0, 100),
  ...
  async evaluateFlag(key: string, environment: string, subjectKey?: string) {
    ...
    const bucket =
      Number.parseInt(
        createHash('sha256')
          .update(`${key}:${environment}:${subjectKey}`)
          .digest('hex')
          .slice(0, 8),
        16,
      ) % 100;
  ...
  async createImportProfile(
    ...
    validateMapping(input.columnMapping);
    validateMapping(input.fieldMapping);
    const row = await this.roadmap.importProfile.create({
      ...
      format: input.format,
      columnMapping: sanitize(input.columnMapping),
      fieldMapping: sanitize(input.fieldMapping),
  ...
  listImportProfiles(entity?: string, active?: boolean) {
    return this.roadmap.importProfile.list(entity, active);
  }
  ...
  async credentials(uuid: string, credentialType?: string) {
    ...
    return (
      await this.roadmap.credential.list(integration.id, credentialType)
    ).map((row) => ({ ...row, secretRef: redact(row.secretRef) }));
  }
  ...
  async revokeCredential(actorUuid: string, credentialUuid: string) {
    const row = await this.roadmap.credential.revoke(
      credentialUuid,
      new Date(),
    );
  ...
  async runtime(uuid: string) {
    const integration = await this.integration(uuid);
    return this.roadmap.runtime.getOrCreate(integration.id);
  }
  ...
  async configureRuntime(
    ...
    return this.roadmap.runtime.update(integration.id, {
      ...
      requestMapping: input.requestMapping
        ? sanitize(input.requestMapping)
        : current.requestMapping,
      responseMapping: input.responseMapping
        ? sanitize(input.responseMapping)
  ...
  async operation(
    ...
    if (runtime.circuitState === 'OPEN')
      await this.roadmap.runtime.update(integration.id, {
    ...
    const row = await this.roadmap.operation.create({
      uuid: randomUUID(),
      integrationId: integration.id,
      ...
      state: 'RUNNING',
      requestHash: hashJson(input.requestPayload ?? {}),
      responseHash: null,
      requestPayload: sanitize(input.requestPayload ?? {}),
  ...
  listOperations(uuid: string, state?: string, limit = 50) {
    return this.integration(uuid).then((row) =>
      this.roadmap.operation.list(row.id, state, limit),
    );
  }
  ...
  async completeOperation(
    ...
    const row = await this.roadmap.operation.update(operationUuid, {
      state: 'SUCCEEDED',
    ...
    await this.roadmap.runtime.update(row.integrationId, {
      circuitState: 'CLOSED',
      failureCount: 0,
      successCount: 1,
      openedAt: null,
  ...
  async failOperation(
    ...
    const row = await this.roadmap.operation.update(operationUuid, {
      state: input.retryable === false ? 'FAILED' : 'RETRY_SCHEDULED',
      nextAttemptAt: input.retryable === false ? null : retryAt(1),
      errorCode: input.code ?? 'INTEGRATION_OPERATION_FAILED',
      errorMessage: input.message.slice(0, 500),
    });
    const current = await this.roadmap.runtime.getOrCreate(row.integrationId);
    const failures = current.failureCount + 1;
    const open = failures >= 5;
    const runtime = await this.roadmap.runtime.update(row.integrationId, {
      failureCount: failures,
      circuitState: open ? 'OPEN' : current.circuitState,
      openedAt: open ? new Date() : current.openedAt,
      nextRetryAt: open ? new Date(Date.now() + 30000) : retryAt(failures),
      lastOperationAt: new Date(),
      lastOperationStatus: 'FAILED',
    });
    if (open)
      await this.roadmap.alert.upsert({
        uuid: randomUUID(),
        alertKey: 'integration.circuit.open',
        severity: 'CRITICAL',
        status: 'OPEN',
        message: `Integration circuit opened after ${failures} failures`,
        dedupeKey: `integration.circuit.open:${row.integrationId}`,
        resourceType: 'system_integration',
        resourceUuid: null,
        metadata: {},
  ...
  async emitEvent(
    ...
    const integration = await this.integration(uuid);
    const existing = await this.roadmap.event.getByKey(
      integration.id,
      input.eventKey,
    );
    if (existing) return existing;
    const payload = sanitize(input.payload);
    return this.roadmap.event.create({
      uuid: randomUUID(),
      integrationId: integration.id,
  ...
  async recordConflict(
    ...
    const integration = await this.integration(uuid);
    return this.roadmap.conflict.upsert({
      uuid: randomUUID(),
      integrationId: integration.id,
      ...
      resolution: 'MANUAL',
      status: 'OPEN',
      localPayload: sanitize(input.localPayload ?? {}),
      remotePayload: sanitize(input.remotePayload ?? {}),
      resolvedBy: null,
      resolvedAt: null,
    });
  }
  ...
  async resolveConflict(
    ...
    const integration = await this.integration(uuid);
    return this.roadmap.conflict.resolve(integration.id, conflictKey, {
      resolution,
  ...
  resync(
    ...
    return this.operation(actorUuid, uuid, {
      operationKey: `manual-resync:${entityType ?? 'all'}`,
      direction,
      idempotencyKey: `manual:${uuid}:${actorUuid}:${Date.now()}`,
      requestPayload: { entityType: entityType ?? null, manual: true },
      maxAttempts: 5,
    });
  }
