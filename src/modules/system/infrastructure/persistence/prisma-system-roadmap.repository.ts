...
export class PrismaSystemRoadmapRepository implements SystemRoadmapRepository {
  ...
  credential = {
    get: async (uuid: string) => this.prisma.systemIntegrationCredential.findUnique({ where: { uuid } }).then((row) => row ? { ...row, metadata: object(row.metadata) } : null),
    ...
    rotate: async (uuid: string, input: any) => this.prisma.$transaction(async (tx) => {
      ...
      return tx.systemIntegrationCredential.create({ data: { uuid: randomUUID(), integrationId: current.integrationId, credentialType: current.credentialType, secretRef: input.secretRef ?? current.secretRef, accessTokenRef: input.accessTokenRef ?? null, refreshTokenRef: input.refreshTokenRef ?? null, version: current.version + 1, status: 'ACTIVE', accessTokenExpiresAt: input.accessTokenExpiresAt ?? null, refreshTokenExpiresAt: input.refreshTokenExpiresAt ?? null, metadata: input.metadata ?? {} } });
    }).then((row) => ({ ...row, metadata: object(row.metadata) })),
  };
  runtime = {
    getOrCreate: async (integrationId: bigint) => { const row = await this.prisma.systemIntegrationRuntime.findUnique({ where: { integrationId } }); return row ? { ...row, requestMapping: object(row.requestMapping), responseMapping: object(row.responseMapping), metadata: object(row.metadata) } : this.prisma.systemIntegrationRuntime.create({ data: { uuid: randomUUID(), integrationId, requestMapping: {}, responseMapping: {}, metadata: {} } }).then((x) => ({ ...x, requestMapping: object(x.requestMapping), responseMapping: object(x.responseMapping), metadata: object(x.metadata) })); },
  ...
  idempotency = {
    reserve: async (input: any) => { try { const row = await this.prisma.systemIntegrationIdempotency.create({ data: { uuid: input.uuid, integrationId: input.integrationId, eventKey: input.eventKey, eventName: input.eventName, eventVersion: input.eventVersion, payloadHash: input.payloadHash, status: input.status, attempt: input.attempt, processedAt: input.processedAt ?? null, lastErrorCode: input.lastErrorCode ?? null } }); return { record: row, created: true }; } catch (error) { if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002') { const row = await this.prisma.systemIntegrationIdempotency.findUniqueOrThrow({ where: { integrationId_eventKey_eventName_eventVersion: { integrationId: input.integrationId, eventKey: input.eventKey, eventName: input.eventName, eventVersion: input.eventVersion } } }); return { record: row, created: false }; } throw error; } },
  ...
  async aggregate() {
    const [featureFlags, importProfiles, integrations, openConflicts, openAlerts, runningOperations, pendingEvents] = await Promise.all([this.prisma.systemFeatureFlag.count(), this.prisma.systemImportProfile.count(), this.prisma.systemIntegration.groupBy({ by: ['state'], _count: { _all: true } }), this.prisma.systemIntegrationConflict.count({ where: { status: 'OPEN' } }), this.prisma.systemOperationalAlert.count({ where: { status: 'OPEN' } }), this.prisma.systemIntegrationOperation.count({ where: { state: { in: ['QUEUED','RUNNING','RETRY_SCHEDULED'] } } }), this.prisma.systemIntegrationEvent.count({ where: { status: { in: ['PENDING','RECEIVED'] } } })]);
