...
export class PrismaSystemRoadmapRepository implements SystemRoadmapRepository {
  ...
  featureFlag = {
    list: async (environment?: string) =>
      this.prisma.systemFeatureFlag
        .findMany({
          where: environment ? { environment } : {},
          orderBy: { key: 'asc' },
        })
        .then((rows) =>
          rows.map((row) => ({ ...row, metadata: object(row.metadata) })),
        ),
    get: async (key: string, environment: string) =>
      this.prisma.systemFeatureFlag
        .findUnique({ where: { key_environment: { key, environment } } })
        .then((row) =>
          row ? { ...row, metadata: object(row.metadata) } : null,
        ),
  ...
  importProfile = {
    ...
    update: async (uuid: string, input: any) =>
      this.prisma.systemImportProfile
        .update({ where: { uuid }, data: input })
        .then((row) => ({
          ...row,
          columnMapping: object(row.columnMapping),
          fieldMapping: object(row.fieldMapping),
  ...
  credential = {
    get: async (uuid: string) =>
      this.prisma.systemIntegrationCredential
        .findUnique({ where: { uuid } })
        .then((row) =>
          row ? { ...row, metadata: object(row.metadata) } : null,
        ),
    ...
    revoke: async (uuid: string, revokedAt: Date) =>
      this.prisma.systemIntegrationCredential
        .update({ where: { uuid }, data: { status: 'REVOKED', revokedAt } })
        .then((row) => ({ ...row, metadata: object(row.metadata) })),
    markUsed: async (uuid: string, lastUsedAt: Date) =>
      this.prisma.systemIntegrationCredential
        .update({ where: { uuid }, data: { lastUsedAt } })
        .then((row) => ({ ...row, metadata: object(row.metadata) })),
    rotate: async (uuid: string, input: any) =>
      this.prisma
  ...
  runtime = {
    getOrCreate: async (integrationId: bigint) => {
      ...
      return row
        ...
        : this.prisma.systemIntegrationRuntime
            .create({
              data: {
                uuid: randomUUID(),
                integrationId,
            ...
            .then((x) => ({
              ...x,
              requestMapping: object(x.requestMapping),
              responseMapping: object(x.responseMapping),
              metadata: object(x.metadata),
            }));
    },
    update: async (integrationId: bigint, input: any) =>
      this.prisma.systemIntegrationRuntime
        .update({ where: { integrationId }, data: input })
        .then((row) => ({
          ...row,
          requestMapping: object(row.requestMapping),
          responseMapping: object(row.responseMapping),
  ...
  operation = {
    getByIdempotency: async (integrationId: bigint, idempotencyKey: string) =>
      this.prisma.systemIntegrationOperation
        .findUnique({
          where: {
  ...
  event = {
    create: async (input: any) =>
      this.prisma.systemIntegrationEvent
        .create({ data: input })
        .then((row) => ({ ...row, payload: object(row.payload) })),
    getByKey: async (integrationId: bigint, eventKey: string) =>
      this.prisma.systemIntegrationEvent
        .findUnique({
          where: { integrationId_eventKey: { integrationId, eventKey } },
        })
        .then((row) => (row ? { ...row, payload: object(row.payload) } : null)),
    update: async (uuid: string, input: any) =>
      this.prisma.systemIntegrationEvent
        .update({ where: { uuid }, data: input })
        .then((row) => ({ ...row, payload: object(row.payload) })),
    list: async (integrationId: bigint, status?: string, limit = 50) =>
      this.prisma.systemIntegrationEvent
        ...
        .then((rows) =>
          rows.map((row) => ({ ...row, payload: object(row.payload) })),
        ),
  };
  idempotency = {
  ...
  conflict = {
    upsert: async (input: any) =>
      this.prisma.systemIntegrationConflict
        .upsert({
          where: {
    ...
    resolve: async (integrationId: bigint, conflictKey: string, input: any) =>
      this.prisma.systemIntegrationConflict
        .update({
          where: { integrationId_conflictKey: { integrationId, conflictKey } },
          data: {
  ...
  alertRule = {
    list: async (enabled?: boolean) =>
      this.prisma.systemOperationalAlertRule
        .findMany({
  ...
  alert = {
    upsert: async (input: any) =>
      this.prisma.systemOperationalAlert
        .upsert({
