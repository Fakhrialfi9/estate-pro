...
export class PrismaSystemRoadmapRepository implements SystemRoadmapRepository {
  ...
  credential = {
    ...
    rotate: async (uuid: string, input: any) => { const current = await this.prisma.systemIntegrationCredential.findUniqueOrThrow({ where: { uuid } }); const version = await this.prisma.systemIntegrationCredential.update({ where: { uuid }, data: { status: 'ROTATED', rotatedAt: new Date() } }); void version; return this.prisma.systemIntegrationCredential.create({ data: { uuid: randomUUID(), integrationId: current.integrationId, credentialType: current.credentialType, secretRef: input.secretRef, version: current.version + 1, status: 'ACTIVE', accessTokenExpiresAt: input.accessTokenExpiresAt ?? null, refreshTokenExpiresAt: input.refreshTokenExpiresAt ?? null, metadata: input.metadata ?? {} } }).then(row => ({ ...row, metadata: object(row.metadata) })); },
  };
