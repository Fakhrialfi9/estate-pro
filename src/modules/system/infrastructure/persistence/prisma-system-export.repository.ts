...
const toRecord = (row: {
  uuid: string; actorUuid: string; entity: string; format: string; state: string; filters: unknown;
  artifactPath: string | null; downloadTokenHash: string | null; rows: number; estimatedRows: number | null;
  processedRows: number; completedAt: Date | null; cancelledAt: Date | null; cancelRequested: boolean;
  artifactBytes: bigint | null; expiresAt: Date; errorMessage: string | null; createdAt: Date; updatedAt: Date;
...
export class PrismaSystemExportRepository implements SystemExportRepository {
  ...
  async create(input: { uuid: string; actorUuid: string; entity: 'system_activity'; format: ExportFormat; filters: Record<string, unknown>; expiresAt: Date; estimatedRows?: number | null }) {
    const row = await this.prisma.systemExportJob.create({ data: {
      ...
      artifactPath: null, downloadTokenHash: null, errorMessage: null,
    } });
    return toRecord(row);
  }
  ...
  async claimQueued(workerId: string) {
    const candidate = await this.prisma.systemExportJob.findFirst({
      where: { state: 'QUEUED', expiresAt: { gt: new Date() } },
      orderBy: [{ createdAt: 'asc' }, { id: 'asc' }],
    });
    if (!candidate) return null;
    const claimed = await this.prisma.systemExportJob.updateMany({
      where: { uuid: candidate.uuid, state: 'QUEUED' },
      data: { state: 'RUNNING', errorMessage: workerId.slice(0, 500) },
    });
  ...
  async listExpired(now: Date, limit: number) {
    const rows = await this.prisma.systemExportJob.findMany({
      where: { expiresAt: { lte: now }, state: { in: ['SUCCEEDED', 'FAILED', 'CANCELLED'] } },
