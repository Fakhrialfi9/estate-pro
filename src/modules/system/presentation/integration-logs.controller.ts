...
export class IntegrationLogsController {
  ...
  @ApiOperation({ summary: 'List bounded, redacted integration lifecycle logs' })
  @ApiResponse({ status: 200, schema: { type: 'array', items: { type: 'object', additionalProperties: true } } })
  list(@Query('integrationUuid') integrationUuid?: string, @Query('state') state?: string, @Query('operationKey') operationKey?: string, @Query('limit') limit?: string) {
    return this.logs.list({ integrationUuid, state, operationKey, limit: limit ? Number(limit) : undefined });
  }
}
