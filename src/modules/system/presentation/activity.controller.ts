...
export class ActivityController {
  ...
  list(@Query() query:ActivityQueryDto){return this.activity.list(query).then(result=>({items:result.items,meta:{page:query.page,limit:query.limit,total:result.total,totalPages:Math.ceil(result.total/query.limit)}}));}
  @Get(':uuid')
  @RequirePermissions('system.activity.read')
  @ApiOperation({summary:'Get a single system activity entry'})
  @ApiResponse({status:200,description:'System activity returned.'})
  get(@Param('uuid',ParseUUIDPipe) uuid:string){return this.activity.get(uuid);}
}
