...
export class ProductionHardeningController {
  ...
  @RequirePermissions('system.integration.read')
  @ApiOperation({ summary: 'Read normalized health for registered integrations' })
  @ApiResponse({ status: 200, schema: { type: 'object', additionalProperties: true } })
  integrationHealth() {
  ...
  @ApiOperation({ summary: 'Retry failed integration operations with dry-run by default' })
  @ApiResponse({ status: 200, schema: { type: 'object', additionalProperties: true } })
  retryFailed(
    @Req() request: Request,
  ...
  @ApiOperation({ summary: 'Inspect or clean explicitly eligible orphaned integration operations' })
  @ApiResponse({ status: 200, schema: { type: 'object', additionalProperties: true } })
  orphanCleanup(
    @Req() request: Request,
