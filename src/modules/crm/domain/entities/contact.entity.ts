...
export type ContactPatch = Partial<
  Pick<
    ContactProps,
    'firstName' | 'lastName' | 'displayName' | 'companyName' | 'jobTitle'
  >
>;
const UUID_RE =
...
export class ContactEntity {
  ...
  static create(props: ContactProps): ContactEntity {
    ...
    if (props.status === 'ARCHIVED' && props.archivedAt === null)
      throw new Error('Archived contact requires archivedAt');
  ...
  update(patch: ContactPatch, now: Date = new Date()): ContactEntity {
    if (this.props.status === 'ARCHIVED')
      throw new Error('Archived contact cannot be mutated');
