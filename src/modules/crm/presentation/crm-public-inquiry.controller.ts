...
export class CrmPublicInquiryController {
 ...
 @Post('inquiries') @Public() @ApiOperation({summary:'Public CRM inquiry intake'}) @ApiResponse({status:201}) @ApiResponse({status:400}) @ApiResponse({status:413}) @ApiResponse({status:429}) submit(@Body()dto:InquiryDto,@Headers('user-agent')ua?:string,@Headers('x-request-id')rid?:string){if(dto.website?.trim())return {data:null};return this.service.inquiry({...dto},undefined).then(value=>({data:value}));}
}
