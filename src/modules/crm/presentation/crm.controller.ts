...
import {
  ...
  TemplateDto,
  TemplatePatchDto,
  ContactChildPatchDto,
} from './crm.dto.js';
import type { CrmActor } from '../domain/crm.types.js';

const actorOf = (req: Request, ua?: string, rid?: string): CrmActor => ({
  actorUuid: (req.user as { sub?: string } | undefined)?.sub ?? '',
  ipAddress: req.ip,
  userAgent: ua,
  requestId: rid,
});
    ...
    : typeof v === 'bigint'
      ...
      : Array.isArray(v)
        ...
        : v && typeof v === 'object'
          ...
          : v;
const response = (v: unknown) => ({ data: out(v) });
const list = (v: {
  ...
  meta: {
    ...
    limit: v.limit,
    total: v.total,
    totalPages: Math.ceil(v.total / v.limit),
  },
});
const childKind = (kind: string): 'address' | 'phone' | 'email' => {
  if (kind === 'address' || kind === 'phone' || kind === 'email') return kind;
  throw new BadRequestException('Invalid contact child type');
};
const configKind = (
  kind: string,
): 'source' | 'campaign' | 'type' | 'status' | 'tag' => {
  if (
    kind === 'source' ||
    kind === 'campaign' ||
    kind === 'type' ||
...
export class CrmController {
  ...
  createContact(
    ...
    return this.service
      .createContact({ ...d }, actorOf(r, ua, rid))
      .then(response);
  }
  @Get('contacts')
  ...
  getContact(@Param('uuid', new ParseUUIDPipe({ version: '4' })) u: string) {
    return this.service.getContact(u).then(response);
  }
  @Patch('contacts/:uuid')
  ...
  updateChild(
    @Req() r: Request,
    @Param('contactUuid', new ParseUUIDPipe({ version: '4' }))
    contactUuid: string,
    @Param('kind') kindValue: string,
    @Param('uuid', new ParseUUIDPipe({ version: '4' })) u: string,
    @Body() d: ContactChildPatchDto,
  ) {
    return this.service
  ...
  primary(
    @Req() r: Request,
    @Param('uuid', new ParseUUIDPipe({ version: '4' })) c: string,
    @Param('kind') kindValue: string,
    @Param('childUuid', new ParseUUIDPipe({ version: '4' })) u: string,
  ) {
    return this.service
  ...
  tag(
    @Req() r: Request,
    @Param('uuid', new ParseUUIDPipe({ version: '4' })) u: string,
    @Param('tagUuid', new ParseUUIDPipe({ version: '4' })) t: string,
  ) {
  ...
  templateUpdate(
    @Req() r: Request,
    @Param('uuid', new ParseUUIDPipe({ version: '4' })) u: string,
    @Body() d: TemplatePatchDto,
  ) {
    return this.service.templateUpdate(u, { ...d }, actorOf(r)).then(response);
  }
}
