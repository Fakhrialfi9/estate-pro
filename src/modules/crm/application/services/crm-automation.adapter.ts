...
export class CrmAutomationAdapter implements AutomationCrmPort {
  ...
  async getLead(uuid: string): Promise<AutomationLeadContext> {
    ...
    return {
      ...
      ownerUserUuid: typeof lead.ownerUserUuid === 'string' ? lead.ownerUserUuid : null,
  ...
  changeLeadStatus(uuid: string, statusUuid: string, actor: CrmActor) {
    return this.crm.changeStatus(uuid, statusUuid, actor).then(asRecord);
  }
}
