...
export class DuplicateDetector {
  detect(source:DuplicateCandidate,candidates:readonly DuplicateCandidate[]):readonly DuplicateMatch[]{
    ...
    return candidates.filter(c=>c.leadUuid!==source.leadUuid).map(candidate=>{const signals:string[]=[];let confidence=0;if(email&&candidate.email&&normalizeEmail(candidate.email)===email){confidence+=60;signals.push('EMAIL');}if(phone&&candidate.phone&&normalizePhone(candidate.phone)===phone){confidence+=30;signals.push('PHONE');}if(name&&candidate.displayName?.trim().toLocaleLowerCase()===name){confidence+=20;signals.push('NAME');}return {candidate,confidence,signals};}).filter(x=>x.confidence>=40).map(x=>({pairKey:duplicatePairKey(source.leadUuid,x.candidate.leadUuid),candidateLeadUuid:x.candidate.leadUuid,confidence:x.confidence,signals:x.signals}));
  }
}
