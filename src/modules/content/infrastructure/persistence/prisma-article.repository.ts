...
export class PrismaArticleRepository {
      ...
      return await this.prisma.$transaction(async (tx) => {
        ...
        const article = await tx.contentArticle.create({
          data: {
            ...
            readingTimeMin: Number(input.readingTimeMin ?? 1),
    ...
    try {
      return await this.prisma.$transaction(async (tx) => {
        ...
        if (input.allowComments !== undefined)
          data.allowComments = Boolean(input.allowComments);
        if (input.wordCount !== undefined)
          data.wordCount = Number(input.wordCount);
        if (input.readingTimeMin !== undefined)
          data.readingTimeMin = Number(input.readingTimeMin);
  ...
  async restore(uuid: string, ctx: AuditContext): Promise<ArticleRecord> {
    const current = await this.prisma.contentArticle.findFirst({
      where: { uuid },
      include: this.include(),
    });
    if (!current) throw new ContentNotFoundError('Article not found');
  ...
  private async upsertSeo(
    ...
    await tx.contentSeo.upsert({
      ...
      update: {
        metaTitle: this.s(value.metaTitle),
        metaDescription: this.s(value.metaDescription),
        keywords: this.s(value.keywords),
        canonicalUrl: this.s(value.canonicalUrl),
        robots: this.s(value.robots),
        ogTitle: this.s(value.ogTitle),
        ogDescription: this.s(value.ogDescription),
        ogImageUrl: this.s(value.ogImageUrl),
        twitterTitle: this.s(value.twitterTitle),
        twitterDescription: this.s(value.twitterDescription),
        twitterImageUrl: this.s(value.twitterImageUrl),
        structuredData:
  ...
  private s(value: unknown): string | null {
