import { IsArray, IsBoolean, IsDateString, IsDefined, IsEmail, IsEnum, IsInt, IsObject, IsOptional, IsString, IsUrl, Max, MaxLength, Min, MinLength, IsUUID } from 'class-validator';
import { CONTENT_FORMATS, CONTENT_STATUSES, VISIBILITIES, ARTICLE_TYPES } from '../../domain/content.types.js';
...
export class ArticleCreateDto {
  ...
  @IsOptional() @IsEnum(CONTENT_FORMATS) contentFormat?: typeof CONTENT_FORMATS[number];
  @IsOptional() @IsEnum(ARTICLE_TYPES) type?: typeof ARTICLE_TYPES[number];
  @IsOptional() @IsEnum(VISIBILITIES) visibility?: typeof VISIBILITIES[number];
...
export class ContentQueryDto {
  ...
  @IsOptional() @IsEnum(CONTENT_STATUSES) status?: typeof CONTENT_STATUSES[number];
  @IsOptional() @IsString() @MaxLength(12) language?: string;
...
export class ResourceDto {
  ...
  @IsOptional() @IsUrl({ protocols: ['http', 'https'], require_protocol: true }) @MaxLength(1000) avatarUrl?: string;
  ...
  @IsOptional() @IsUrl({ protocols: ['http', 'https'], require_protocol: true }) @MaxLength(1000) linkUrl?: string;
...
export class PublishDto { @IsOptional() @IsDateString() scheduledAt?: string; }
export class ReorderMenuDto { @IsArray() @IsUUID('4', { each: true }) itemUuids!: string[]; }
export class RelationDto { @IsUUID() sourceUuid!: string; @IsUUID() targetUuid!: string; @IsString() sourceType!: string; @IsString() targetType!: string; @IsOptional() @IsString() relationType?: string; @IsOptional() @IsInt() @Min(0) sortOrder?: number; }
export class CommentDto { @IsDefined() content!: unknown; @IsOptional() @IsString() @MaxLength(180) authorName?: string; @IsOptional() @IsEmail() @MaxLength(320) authorEmail?: string; @IsOptional() @IsUUID() parentUuid?: string; }
export class ModerationDto { @IsEnum(['APPROVED', 'REJECTED', 'SPAM', 'DELETED']) status!: 'APPROVED' | 'REJECTED' | 'SPAM' | 'DELETED'; @IsOptional() @IsString() @MaxLength(500) reason?: string; }
