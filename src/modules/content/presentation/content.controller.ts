...
export class ContentController {
  ...
  private ctx(req: AuthRequest, ua?: string, requestId?: string) {
    return {
  ...
  async remove(
    ...
    try {
      await this.deleteArticle.execute(uuid, this.ctx(req, ua, id));
    } catch (e) {
      this.map(e);
    }
  }
  ...
  @RequirePermissions('content.articles.restore')
  @ApiOperation({ summary: 'Restore revision' })
  ...
  private async resource(
    ...
    query: ContentQueryDto | undefined,
    action: 'list' | 'get' | 'create' | 'update' | 'delete' | 'restore',
  ) {
    const permission = RESOURCE_PERMISSIONS[resource];
    if (action === 'list') return this.resources.list(resource, query ?? {});
    if (action === 'get') return this.resources.get(resource, uuid!, false);
    if (action === 'create')
      return this.resources.create(resource, body ?? {}, this.ctx(req));
    if (action === 'update')
      return this.resources.update(resource, uuid!, body ?? {}, this.ctx(req));
    if (action === 'delete')
      return this.resources.delete(resource, uuid!, this.ctx(req));
    return this.resources.restore(resource, uuid!, this.ctx(req));
  }

  @Get(':resource')
  @RequirePermissions('content.management.read')
  @ApiOperation({ summary: 'List CMS resource' })
  async resourceList(
    ...
    return this.resources.list(
      resource as Exclude<
        keyof typeof RESOURCE_PERMISSIONS,
  ...
  @Get(':resource/:uuid')
  @RequirePermissions('content.management.read')
  @ApiOperation({ summary: 'Get CMS resource' })
  async resourceGet(
    @Param('resource') resource: string,
    @Param('uuid') uuid: string,
  ) {
  ...
  @Post(':resource')
  @RequirePermissions('content.management.create')
  @ApiOperation({ summary: 'Create CMS resource' })
  ...
  @Patch(':resource/:uuid')
  @RequirePermissions('content.management.update')
  @ApiOperation({ summary: 'Update CMS resource' })
  ...
  @RequirePermissions('content.management.delete')
  @HttpCode(204)
  @ApiOperation({ summary: 'Delete CMS resource' })
  ...
  @Post(':resource/:uuid/restore')
  @RequirePermissions('content.management.restore')
  @ApiOperation({ summary: 'Restore CMS resource' })
  async resourceRestore(
    @Req() req: AuthRequest,
    @Param('resource') resource: string,
    @Param('uuid') uuid: string,
  ) {
    if (
      ...
      throw new NotFoundException('CMS resource not found');
    try {
      return await this.resources.restore(
        resource as Exclude<
          keyof typeof RESOURCE_PERMISSIONS,
          'article' | 'media' | 'comment'
        >,
  ...
  @HttpCode(204)
  async mediaDelete(@Req() req: AuthRequest, @Param('uuid') uuid: string) {
    try {
      await this.media.remove(uuid, this.ctx(req));
    } catch (e) {
      this.map(e);
    }
  }
  ...
  async reorder(
    ...
    return this.service.reorderMenu(uuid, dto.itemUuids, this.ctx(req));
  }
  @Post('relations')
  @RequirePermissions('content.relations.create')
  async relationCreate(@Req() req: AuthRequest, @Body() dto: RelationDto) {
  ...
  async relationList(
    ...
    return this.service.listRelations(sourceUuid, type);
  }
