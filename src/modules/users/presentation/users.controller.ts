...
export class UsersController {
  ...
  async list(@Query() dto: UserQueryDto) {
    ...
    const result = await this.users.list({ page: dto.page ?? 1, limit: dto.limit ?? 20, ...(dto.filterField !== undefined ? { filterField: dto.filterField as UserFilterField } : {}), ...(dto.filterValue !== undefined ? { filterValue: dto.filterValue } : {}), sortBy: (dto.sortBy ?? 'createdAt') as UserSortField, sortDirection: dto.sortDirection ?? 'desc', ...(dto.search !== undefined ? { search: dto.search.trim() } : {}) });
    return { items: result.items.map(serializeUser), meta: { page: result.page, limit: result.limit, total: result.total, totalPages: Math.ceil(result.total / result.limit) } };
  }

  @Get('email/:email')
  async getByEmail(@Param('email') email: string) { try { return serializeUser(await this.users.getByEmail(email)); } catch (error: unknown) { this.mapError(error); } }
  @Get('username/:username')
  async getByUsername(@Param('username') username: string) { try { return serializeUser(await this.users.getByUsername(username)); } catch (error: unknown) { this.mapError(error); } }
  @Get(':uuid')
  async getByUuid(@Param('uuid') uuid: string) { try { return serializeUser(await this.users.getByUuid(uuid)); } catch (error: unknown) { this.mapError(error); } }
  ...
  async remove(@Req() request: AuthenticatedRequest, @Param('uuid') uuid: string): Promise<void> {
    try { await this.users.remove(uuid, this.auditContext(request)); } catch (error: unknown) { this.mapError(error); }
  }

  private auditContext(request: AuthenticatedRequest): UserAuditContext { return { actorUuid: request.user?.sub, ipAddress: request.ip, userAgent: request.get('user-agent') ?? undefined, requestId: request.get('x-request-id') ?? undefined }; }
  private normalizeStatus(value: string) { const status = value.trim().toLowerCase(); if (!['pending', 'active', 'inactive', 'suspended'].includes(status)) throw new InvalidUserError('Invalid user status'); return status as 'pending' | 'active' | 'inactive' | 'suspended'; }
  private mapError(error: unknown): never { if (error instanceof UserNotFoundError) throw new NotFoundException('User not found'); if (error instanceof DuplicateUserError || (error as { name?: string }).name === 'DuplicateUserError') throw new ConflictException('User identity is already in use'); if (error instanceof InvalidUserError) throw new BadRequestException(error.message); throw error; }
}
