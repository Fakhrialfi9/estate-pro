...
export class UsersController {
  ...
  async list(@Query() dto: UserQueryDto) {
    ...
    if (dto.filterField === 'isActive' && dto.filterValue !== undefined && !['true', 'false'].includes(dto.filterValue.toLowerCase())) {
  ...
  async getByEmail(@Param('email') email: string) {
    try { return serializeUser(await this.users.getByEmail(email)); } catch (error: unknown) { this.mapError(error); }
  }

  @Get('username/:username')
  async getByUsername(@Param('username') username: string) {
    try { return serializeUser(await this.users.getByUsername(username)); } catch (error: unknown) { this.mapError(error); }
  }

  @Get(':uuid')
  async getByUuid(@Param('uuid') uuid: string) {
    try { return serializeUser(await this.users.getByUuid(uuid)); } catch (error: unknown) { this.mapError(error); }
  }
  ...
  async remove(@Param('uuid') uuid: string): Promise<void> {
    try { await this.users.remove(uuid); } catch (error: unknown) { this.mapError(error); }
  }
