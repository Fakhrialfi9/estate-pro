...
export class UserManagementService {
  ...
  async create(data: CreateUserData, context: UserAuditContext = {}): Promise<UserEntity> {
    ...
    await this.audit.record({ action: 'USER_CREATED', actorUuid: context.actorUuid, entityType: 'user', entityUuid: created.uuid, result: 'SUCCESS', ipAddress: context.ipAddress, userAgent: context.userAgent, requestId: context.requestId });
    return created;
  }

  async getByUuid(uuid: string): Promise<UserEntity> { const user = await this.users.findByUuid(uuid); if (!user) throw new UserNotFoundError(); return user; }
  ...
  async update(uuid: string, changes: UserUpdate, context: UserAuditContext = {}): Promise<UserEntity> {
    ...
    if (this.hasSecurityDisablingChange(existing, normalized)) {
      await this.sessions.revokeAllForSecurityEvent(uuid, normalized.isActive === false || normalized.status === 'inactive' ? 'ACCOUNT_DISABLED' : 'SECURITY_STATE_CHANGE', context);
    }
    const auditChanges = [
    ...
    await this.audit.record({ action: 'USER_UPDATED', actorUuid: context.actorUuid, entityType: 'user', entityUuid: updated.uuid, result: 'SUCCESS', changes: auditChanges, ipAddress: context.ipAddress, userAgent: context.userAgent, requestId: context.requestId });
    return updated;
  }
  ...
  async remove(uuid: string, context: UserAuditContext = {}): Promise<void> {
    ...
    await this.sessions.revokeAllForSecurityEvent(uuid, 'ACCOUNT_DISABLED', context);
    await this.audit.record({ action: 'USER_DELETED', actorUuid: context.actorUuid, entityType: 'user', entityUuid: uuid, result: 'SUCCESS', reason: 'SOFT_DELETED', ipAddress: context.ipAddress, userAgent: context.userAgent, requestId: context.requestId });
  }

  private hasSecurityDisablingChange(existing: UserEntity, changes: UserUpdate): boolean {
    return (changes.isActive === false || changes.status === 'inactive' || changes.status === 'suspended') && existing.isAccessible();
  }
