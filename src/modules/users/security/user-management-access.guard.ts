  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    try {
      const claims = await this.jwt.verifyAsync<AuthClaims>(token, {
        ...
        algorithms: [this.config.getOrThrow<'HS256' | 'HS384' | 'HS512'>('auth.jwt.algorithm')],
      });
      ...
      const isOwnResource = typeof pathUuid === 'string' && pathUuid === claims.sub;
      const method = request.method.toUpperCase();
      const requiresManagement = !isOwnResource || !['GET', 'HEAD'].includes(method);

      if (requiresManagement && !permissions.has('users:manage')) throw new ForbiddenException();
      return true;
    } catch (error: unknown) {
