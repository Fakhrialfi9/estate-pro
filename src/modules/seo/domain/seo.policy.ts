...
export const isPubliclyIndexable = (resource: SeoResourceSnapshot): boolean =>
  resource.published &&
  resource.seo.robots !== 'noindex,nofollow' &&
  resource.seo.robots !== 'noindex,follow';

export const buildCanonicalUrl = (baseUrl: string, path: string): string => {
  const base = new URL(baseUrl);
  if (!path.startsWith('/') || path.startsWith('//'))
    throw new Error('SEO canonical path must be internal');
  const url = new URL(path, base);
  if (url.origin !== base.origin)
    throw new Error('SEO canonical URL must use the application origin');
  url.search = '';
  url.hash = '';
  return url.toString();
};

const validateStoredCanonical = (
  baseUrl: string,
...
export const buildSeoMetadata = (
  resource: SeoResourceSnapshot,
  baseUrl: string,
  path: string,
): SeoMetadata => {
  const title =
    normalizeSeoText(resource.seo.title) || normalizeSeoText(resource.title);
  const description =
    normalizeSeoText(resource.seo.description) ||
    normalizeSeoText(resource.description) ||
    title;
  const fallbackCanonical = buildCanonicalUrl(baseUrl, path);
  const canonicalUrl = validateStoredCanonical(
    baseUrl,
    resource.seo.canonicalUrl,
    fallbackCanonical,
  );
  const robots: SeoRobots =
    resource.seo.robots === 'index,nofollow' ||
    resource.seo.robots === 'noindex,follow' ||
    resource.seo.robots === 'noindex,nofollow'
      ? resource.seo.robots
      : 'index,follow';
  ...
  const imageUrl =
    typeof openGraph?.imageUrl === 'string' && openGraph.imageUrl.length > 0
      ? openGraph.imageUrl
      : null;
  return {
    ...
    openGraph: {
      ...
      description:
        normalizeSeoText(openGraph?.description) || description.slice(0, 160),
      url: canonicalUrl,
      imageUrl,
    ...
    twitter: {
      ...
      description:
        normalizeSeoText(twitter?.description) || description.slice(0, 160),
      imageUrl,
    },
    metadataVersion:
      typeof resource.seo.metadataVersion === 'string'
        ? resource.seo.metadataVersion
        : '1.0',
  };
};
