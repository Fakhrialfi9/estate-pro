...
import { IsEnum, IsInt, IsOptional, IsString, IsUrl, IsUUID, Max, MaxLength, Min } from 'class-validator';
...
export class UpdatePropertySeoDto {
  ...
  @IsOptional() @IsUrl({ protocols: ['http', 'https'], require_protocol: true }) canonicalUrl?: string | null;
  @IsOptional() @IsEnum(['INDEX_FOLLOW', 'NOINDEX_FOLLOW', 'INDEX_NOFOLLOW', 'NOINDEX_NOFOLLOW'] as const) robots?: 'INDEX_FOLLOW' | 'NOINDEX_FOLLOW' | 'INDEX_NOFOLLOW' | 'NOINDEX_NOFOLLOW';
  @IsOptional() @IsUrl({ protocols: ['http', 'https'], require_protocol: true }) ogImageUrl?: string | null;
...
export class UpdateContentSeoDto {
  ...
  @IsOptional() @IsUrl({ protocols: ['http', 'https'], require_protocol: true }) twitterImageUrl?: string | null;
}

export class CreateRedirectDto {
  @IsString() @MaxLength(1000) sourcePath!: string;
  @IsString() @MaxLength(1000) destination!: string;
  @IsOptional() @Type(() => Number) @IsInt() @Min(301) @Max(302) statusCode: 301 | 302 = 301;
}
