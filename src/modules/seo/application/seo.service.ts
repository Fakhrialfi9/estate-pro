...
import { buildCanonicalUrl, buildSeoMetadata, isPubliclyIndexable, normalizeSeoText } from '../domain/seo.policy.js';
import type { SeoMetadata, SeoRedirect, SeoResourceSnapshot } from '../domain/seo.types.js';
...
export class SeoService {
  ...
  async getPublicResource(resourceType: SeoResourceSnapshot['resourceType'], identifier: string, language = 'id'): Promise<{ resource: SeoResourceSnapshot; metadata: SeoMetadata }> {
    ...
    return { resource, metadata: buildSeoMetadata(resource, this.baseUrl(), this.resourcePath(resource)) };
  }

  async getPublicMetadata(resourceType: SeoResourceSnapshot['resourceType'], identifier: string, language = 'id') {
  ...
  async getPublicStructuredData(resourceType: SeoResourceSnapshot['resourceType'], identifier: string, language = 'id'): Promise<Record<string, unknown>> {
    ...
    if (resourceType === 'article') return { ...base, '@type': 'Article', headline: metadata.title, datePublished: resource.publishedAt?.toISOString() };
    if (resourceType === 'listing') return { ...base, '@type': 'Offer' };
    return { ...base, '@type': 'WebPage' };
  }

  async updatePropertyMetadata(propertyUuid: string, input: { title?: string | null; description?: string | null; canonicalUrl?: string | null; robots?: 'INDEX_FOLLOW' | 'NOINDEX_FOLLOW' | 'INDEX_NOFOLLOW' | 'NOINDEX_NOFOLLOW'; ogImageUrl?: string | null; metadataVersion?: string }, actorUuid: string) {
    this.validateMetadataInput(input.title, input.description, input.canonicalUrl);
    return this.repository.upsertPropertyMetadata(propertyUuid, {
      ...input,
      ...
      canonicalUrl: input.canonicalUrl === undefined || input.canonicalUrl === null ? input.canonicalUrl : this.validateCanonicalUrl(input.canonicalUrl),
    }, actorUuid);
  }

  async updateContentMetadata(resourceType: 'article' | 'page', resourceUuid: string, input: { metaTitle?: string | null; metaDescription?: string | null; canonicalUrl?: string | null; robots?: string | null; ogTitle?: string | null; ogDescription?: string | null; ogImageUrl?: string | null; twitterTitle?: string | null; twitterDescription?: string | null; twitterImageUrl?: string | null }, actorUuid: string) {
    this.validateMetadataInput(input.metaTitle, input.metaDescription, input.canonicalUrl);
    return this.repository.upsertContentMetadata(resourceType, resourceUuid, { ...input, metaTitle: input.metaTitle === undefined ? undefined : normalizeSeoText(input.metaTitle), metaDescription: input.metaDescription === undefined ? undefined : normalizeSeoText(input.metaDescription), canonicalUrl: input.canonicalUrl === undefined || input.canonicalUrl === null ? input.canonicalUrl : this.validateCanonicalUrl(input.canonicalUrl) }, actorUuid);
  }
  ...
  async sitemapChunk(part: number, chunkSize = 50000): Promise<string> {
    ...
    const resources = (await this.repository.listPublicResources()).filter(isPubliclyIndexable);
    const slice = resources.slice((part - 1) * chunkSize, part * chunkSize);
    if (slice.length === 0) throw new NotFoundException('Sitemap part not found');
    const urls = slice.map((resource) => {
      const metadata = buildSeoMetadata(resource, this.baseUrl(), this.resourcePath(resource));
