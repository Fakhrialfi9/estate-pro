...
import {
  ...
  type MatchingSubjectType,
  type PropertyPreferenceState,
...
const candidateHistogram = meter.createHistogram(
  'property_matching_candidate_count',
);
const durationHistogram = meter.createHistogram(
  'property_matching_duration_ms',
);
...
export class PropertyMatchingService {
  ...
  async getPreference(
    ...
    if (!preference)
      throw new NotFoundException('Matching preference not found');
    return preference;
  }
  ...
  async createPreference(
    ...
    await this.assertSubjectAccess(subjectType, subjectUuid, actor);
    const existing = await this.repository.findPreference(
      subjectType,
      subjectUuid,
    );
    ...
    let result;
    if (!existing)
      result = await this.repository.createPreference(
        subjectType,
        subjectUuid,
        preference,
      );
    else if (existing.status === 'ARCHIVED')
      result = await this.repository.restorePreference(
        subjectType,
        subjectUuid,
        existing.version,
        preference,
    ...
    await this.auditSafe(
      MATCHING_AUDIT_ACTIONS.PREFERENCE_CREATED,
      subjectUuid,
      actor,
      subjectType,
    );
  ...
  async updatePreference(
    ...
    await this.assertSubjectAccess(subjectType, subjectUuid, actor);
    const current = await this.repository.findPreference(
      subjectType,
      subjectUuid,
    );
    ...
    const updated = await this.repository.updatePreference(
      subjectType,
      subjectUuid,
      expectedVersion,
      preference,
    ...
    await this.auditSafe(
      MATCHING_AUDIT_ACTIONS.PREFERENCE_UPDATED,
      subjectUuid,
      actor,
      subjectType,
    );
  ...
  async archivePreference(
    ...
    await this.assertSubjectAccess(subjectType, subjectUuid, actor);
    const result = await this.repository.archivePreference(
      subjectType,
      subjectUuid,
      expectedVersion,
    );
    await this.auditSafe(
      MATCHING_AUDIT_ACTIONS.PREFERENCE_ARCHIVED,
      subjectUuid,
      actor,
      subjectType,
    );
  ...
  async restorePreference(
    ...
    if (
      ...
      throw new ConflictException('Preference version is stale');
    return this.repository.restorePreference(
      subjectType,
      subjectUuid,
      expectedVersion,
  ...
  async match(
    subjectType: MatchingSubjectType,
    subjectUuid: string,
    options: { minScore?: number; page?: number; limit?: number },
    actor: MatchingActor,
  ) {
    await this.assertSubjectAccess(subjectType, subjectUuid, actor);
    const preference = await this.requireActivePreference(
      subjectType,
      subjectUuid,
    );
    ...
    try {
      const pool = await this.repository.listCandidates({
        preference,
        now: new Date(),
        limit: MAX_CANDIDATE_POOL,
      });
      candidateHistogram.record(pool.length);
      const signals = await this.repository.getSignals(
        actor.actorUuid,
  ...
  async generate(
    subjectType: MatchingSubjectType,
    subjectUuid: string,
    source: 'GENERATED' | 'REFRESHED' | 'RECALCULATED',
    ...
    try {
      ...
      await this.auditSafe(
        source === 'GENERATED'
          ? MATCHING_AUDIT_ACTIONS.RECOMMENDATION_GENERATED
          : MATCHING_AUDIT_ACTIONS.RECOMMENDATION_REFRESHED,
        snapshot.uuid,
        actor,
        'RECOMMENDATION',
      );
  ...
  async getLatest(
    ...
    await this.assertSubjectAccess(subjectType, subjectUuid, actor);
    const result = await this.repository.getLatestRecommendation(
      subjectType,
      subjectUuid,
    );
  ...
  async getHistory(
    ...
    const result = await this.repository.listRecommendationHistory(
      subjectType,
      subjectUuid,
      safePage,
      safeLimit,
  ...
  async submitFeedback(
    ...
    await this.auditSafe(
      MATCHING_AUDIT_ACTIONS.FEEDBACK_RECORDED,
      input.recommendationItemUuid,
      actor,
      'FEEDBACK',
    );
    return result;
  ...
  private async requireActivePreference(
    ...
    if (!preference || preference.status !== 'ACTIVE')
      throw new NotFoundException('Active matching preference not found');
    return preference;
  }
  ...
  private async assertSubjectAccess(
    ...
    const scope = await this.repository.getPreferenceSubjectScope(
      subjectType,
      subjectUuid,
    );
    ...
    const permission =
      subjectType === 'CONTACT' ? 'crm.contacts.read' : 'crm.leads.read';
    ...
    try {
      ...
      this.authorization.assertPermissions(snapshot, ['sales.manage'], 'OR');
    }
  }
