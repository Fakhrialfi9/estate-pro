...
export interface AutomationRepository {
  ...
  createExecution(input: Record<string, unknown>): Promise<Record<string, unknown>>;
  getExecution(uuid: string): Promise<Record<string, unknown> | null>;
  listExecutions(input: Record<string, unknown>): Promise<{ items: readonly Record<string, unknown>[]; total: number; page: number; limit: number }>;
  updateExecution(uuid: string, input: Record<string, unknown>): Promise<Record<string, unknown>>;
  createAction(input: Record<string, unknown>): Promise<Record<string, unknown>>;
  getAction(uuid: string): Promise<Record<string, unknown> | null>;
  listActions(executionUuid: string): Promise<readonly Record<string, unknown>[]>;
  updateAction(uuid: string, input: Record<string, unknown>): Promise<Record<string, unknown>>;
  claimDueExecution(workerId: string, leaseMs: number): Promise<Record<string, unknown> | null>;
  claimDueAction(workerId: string, leaseMs: number): Promise<Record<string, unknown> | null>;
  reclaimExpired(workerId: string, now: Date): Promise<number>;
  countRecentActionExecutions(workflowUuid: string, entityUuid: string, since: Date): Promise<number>;
  createAssignmentRule(input: Record<string, unknown>): Promise<Record<string, unknown>>;
  createSlaPolicy(input: Record<string, unknown>): Promise<Record<string, unknown>>;
  createSlaInstance(input: Record<string, unknown>): Promise<Record<string, unknown>>;
  claimDueSla(workerId: string, leaseMs: number): Promise<Record<string, unknown> | null>;
...
export interface ActionHandler {
  ...
  execute(input: Record<string, unknown>, context: Record<string, unknown>, actorUuid: string): Promise<{
    success: boolean;
    retryable: boolean;
    reference?: string;
