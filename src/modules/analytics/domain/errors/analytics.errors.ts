...
export class AnalyticsScopeException extends ForbiddenException {
  constructor(message = 'You are not authorized to access this analytics scope.') {
...
export class AnalyticsQueryTimeoutException extends GatewayTimeoutException {
  constructor() {
    super({ code: 'ANALYTICS_QUERY_TIMEOUT', message: 'Analytics query timed out.' });
  }
}

export class AnalyticsUnavailableException extends ServiceUnavailableException {
  constructor() {
    super({ code: 'ANALYTICS_UNAVAILABLE', message: 'Analytics data is temporarily unavailable.' });
  }
}
