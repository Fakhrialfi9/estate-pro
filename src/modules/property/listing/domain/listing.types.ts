...
export function assertListingTransition(
  ...
  if (!LISTING_STATUSES.includes(to))
    throw new Error(`Unsupported listing status: ${to}`);
...
export function assertPricingInvariants(price: ListingPricingInput): void {
  ...
  const min =
    price.minPrice == null ? null : toScaledInteger(price.minPrice, 2);
  const max =
    price.maxPrice == null ? null : toScaledInteger(price.maxPrice, 2);
  if (min === 0n || max === 0n)
    throw new Error('Price must be greater than zero');
  if (min !== null && max !== null && min > max)
    throw new Error('Minimum price must not exceed maximum price');
  if (price.pricePerSqm != null && toScaledInteger(price.pricePerSqm, 4) === 0n)
    throw new Error('Price per sqm must be greater than zero');
  if (min === null && max === null && price.pricePerSqm == null)
...
export function assertPaymentInvariants(payment: ListingPaymentInput): void {
  if (payment.downPaymentAmount != null)
    toScaledInteger(payment.downPaymentAmount, 2);
  if (payment.installmentAmount != null) {
    if (toScaledInteger(payment.installmentAmount, 2) === 0n)
  ...
  if (
    payment.tenorMonths != null &&
    (!Number.isInteger(payment.tenorMonths) || payment.tenorMonths <= 0)
  )
    throw new Error('Tenor must be a positive integer number of months');
  if (
    payment.installmentAmount != null &&
    (payment.tenorMonths == null || payment.tenorMonths <= 0)
  )
