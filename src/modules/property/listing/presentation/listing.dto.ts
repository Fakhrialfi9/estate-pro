...
import { IsBoolean, IsEnum, IsInt, IsISO8601, IsOptional, IsString, IsUUID, Max, MaxLength, Min, MinLength, ValidateNested } from 'class-validator';
import { LISTING_PRICE_TYPES, LISTING_STATUSES, LISTING_TRANSACTION_TYPES, LISTING_VISIBILITIES, PAYMENT_OPTION_TYPES, type ListingPriceType, type ListingStatus, type ListingTransactionType, type ListingVisibility, type PaymentOptionType } from '../domain/listing.types.js';
...
export class ListingPaymentDto {
  ...
  @IsOptional() @Type(() => Number) @IsInt() @Min(1) @Max(600) tenorMonths?: number | null;
...
export class CreateListingDto {
  ...
  @IsOptional() @ValidateNested() @Type(() => ListingPriceDto) price?: ListingPriceDto;
  @IsOptional() @ValidateNested({ each: true }) @Type(() => ListingPaymentDto) payments?: ListingPaymentDto[];
}
export class UpdateListingDto {
  @Type(() => Number) @IsInt() @Min(1) version!: number;
  @IsOptional() @IsString() @MinLength(3) @MaxLength(80) listingCode?: string;
  @IsOptional() @IsEnum(LISTING_TRANSACTION_TYPES) transactionType?: ListingTransactionType;
  @IsOptional() @IsEnum(LISTING_VISIBILITIES) visibility?: ListingVisibility;
  @IsOptional() @IsBoolean() featured?: boolean;
  @IsOptional() @IsBoolean() premium?: boolean;
  @IsOptional() @IsISO8601() expiresAt?: string | null;
...
export class OwnerAssignmentDto {
  @IsEnum(['INDIVIDUAL', 'COMPANY', 'JOINT', 'GOVERNMENT', 'OTHER']) ownerType!: string;
  @IsString() @MinLength(2) @MaxLength(160) ownerDisplayName!: string;
}
const csv = (value: unknown): unknown => typeof value === 'string' ? value.split(',').map((item) => item.trim()).filter(Boolean) : value;
const bool = (value: unknown): unknown => typeof value === 'string' ? value === 'true' : value;
export class PropertySearchDto {
  ...
  @IsOptional() @Transform(({ value }) => csv(value)) @IsUUID('4', { each: true }) facilityUuids?: string[];
  @IsOptional() @IsEnum(LISTING_TRANSACTION_TYPES) transactionType?: ListingTransactionType;
  @IsOptional() @IsEnum(LISTING_STATUSES) listingStatus?: ListingStatus;
  @IsOptional() @Transform(({ value }) => bool(value)) @IsBoolean() featured?: boolean;
  @IsOptional() @Transform(({ value }) => bool(value)) @IsBoolean() verified?: boolean;
  @IsOptional() @IsEnum(['price', 'createdAt', 'updatedAt', 'views', 'featured']) sortBy?: 'price' | 'createdAt' | 'updatedAt' | 'views' | 'featured';
