import {
  IsBoolean, IsDecimal, IsEnum, IsInt, IsOptional, IsString, IsUrl, IsUUID, Matches, Max, MaxLength, Min,
} from 'class-validator';
import {
  COORDINATE_ACCURACIES, CONDITIONS, FURNISHED_STATUSES, MAP_PROVIDERS, NATURAL_LIGHTING, ORIENTATIONS,
  PARKING_TYPES, RISK_LEVELS, ROOM_TYPES, VENTILATION_TYPES,
} from '../../domain/property-details.js';

const NON_NEGATIVE_DECIMAL = { decimal_digits: '0,2', force_decimal: false } as const;
const SIGNED_COORDINATE = { decimal_digits: '0,7', force_decimal: false } as const;
const DECIMAL_PATTERN = /^\d{1,16}(\.\d{1,2})?$/;
const SIGNED_COORDINATE_PATTERN = /^-?\d{1,3}(\.\d{1,7})?$/;

export class PropertySpecificationDto {
  @IsOptional() @IsString() @Matches(DECIMAL_PATTERN) @IsDecimal(NON_NEGATIVE_DECIMAL) landArea?: string;
  @IsOptional() @IsString() @Matches(DECIMAL_PATTERN) @IsDecimal(NON_NEGATIVE_DECIMAL) buildingArea?: string;
  @IsOptional() @IsString() @Matches(DECIMAL_PATTERN) @IsDecimal(NON_NEGATIVE_DECIMAL) floorArea?: string;
  @IsOptional() @IsInt() @Min(0) @Max(1000) bedrooms?: number;
  @IsOptional() @IsString() @Matches(DECIMAL_PATTERN) @IsDecimal(NON_NEGATIVE_DECIMAL) bathrooms?: string;
  ...
  @IsOptional() @IsEnum(PARKING_TYPES) parkingType?: (typeof PARKING_TYPES)[number];
  ...
  @IsOptional() @IsEnum(ORIENTATIONS) orientation?: (typeof ORIENTATIONS)[number];
  @IsOptional() @IsEnum(CONDITIONS) condition?: (typeof CONDITIONS)[number];
  @IsOptional() @IsEnum(FURNISHED_STATUSES) furnishedStatus?: (typeof FURNISHED_STATUSES)[number];
  @IsOptional() @IsString() @Matches(DECIMAL_PATTERN) @IsDecimal(NON_NEGATIVE_DECIMAL) ceilingHeightM?: string | null;
  @IsOptional() @IsString() @Matches(DECIMAL_PATTERN) @IsDecimal(NON_NEGATIVE_DECIMAL) frontageM?: string | null;
  @IsOptional() @IsString() @Matches(DECIMAL_PATTERN) @IsDecimal(NON_NEGATIVE_DECIMAL) roadWidthM?: string | null;
}
...
export class PropertyLocationDto {
  ...
  @IsOptional() @IsString() @Matches(/^[0-9A-Za-z -]{3,20}$/) postalCode?: string | null;
  @IsOptional() @IsString() @Matches(SIGNED_COORDINATE_PATTERN) @IsDecimal(SIGNED_COORDINATE) latitude?: string | null;
  @IsOptional() @IsString() @Matches(SIGNED_COORDINATE_PATTERN) @IsDecimal(SIGNED_COORDINATE) longitude?: string | null;
  @IsOptional() @IsEnum(COORDINATE_ACCURACIES) coordinateAccuracy?: (typeof COORDINATE_ACCURACIES)[number];
  @IsOptional() @IsEnum(MAP_PROVIDERS) mapProvider?: (typeof MAP_PROVIDERS)[number] | null;
  @IsOptional() @IsString() @MaxLength(255) placeId?: string | null;
  @IsOptional() @IsUrl({ protocols: ['http', 'https'], require_protocol: true }) @MaxLength(1000) mapUrl?: string | null;
  @IsOptional() @IsEnum(RISK_LEVELS) floodRisk?: (typeof RISK_LEVELS)[number];
  @IsOptional() @IsEnum(RISK_LEVELS) earthquakeRisk?: (typeof RISK_LEVELS)[number];
  ...
  @IsOptional() @IsEnum(RISK_LEVELS) airQualityRisk?: (typeof RISK_LEVELS)[number];
}
...
export class PropertyBuildingDto {
  ...
  @IsOptional() @IsString() @Matches(DECIMAL_PATTERN) @IsDecimal(NON_NEGATIVE_DECIMAL) poolLengthM?: string | null;
  @IsOptional() @IsString() @Matches(DECIMAL_PATTERN) @IsDecimal(NON_NEGATIVE_DECIMAL) poolWidthM?: string | null;
  @IsOptional() @IsString() @Matches(DECIMAL_PATTERN) @IsDecimal(NON_NEGATIVE_DECIMAL) poolDepthM?: string | null;
  @IsOptional() @IsString() @MaxLength(150) interiorStyle?: string | null;
  @IsOptional() @IsString() @MaxLength(500) interiorDesign?: string | null;
  @IsOptional() @IsEnum(NATURAL_LIGHTING) naturalLighting?: (typeof NATURAL_LIGHTING)[number];
  @IsOptional() @IsEnum(VENTILATION_TYPES) ventilation?: (typeof VENTILATION_TYPES)[number];
...
export class PropertyRoomDto {
  ...
  @IsString() @Matches(/^(0|\d{1,16})(\.\d{1,2})?$/) @IsDecimal(NON_NEGATIVE_DECIMAL) area!: string;
...
export class PropertyRoomUpdateDto {
  ...
  @IsOptional() @IsString() @Matches(/^(0|\d{1,16})(\.\d{1,2})?$/) @IsDecimal(NON_NEGATIVE_DECIMAL) area?: string;
  @IsOptional() @IsBoolean() hasBathroom?: boolean;
  @IsOptional() @IsBoolean() hasWalkInCloset?: boolean;
  @IsOptional() @IsBoolean() hasBalcony?: boolean;
  @IsOptional() @IsBoolean() hasAirConditioning?: boolean;
