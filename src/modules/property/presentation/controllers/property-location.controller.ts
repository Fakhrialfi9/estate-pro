...
const levelOf = (
  ...
  if (
    ...
    return level as ReturnType<typeof levelOf>;
...
export class PropertyLocationController {
  ...
  @Post('locations/:level') @RequirePermissions('locations.manage') create(
  ...
  @Get('locations/:level') @RequirePermissions('locations.read') list(
    @Param('level') level: string,
    @Query() q: ListQuery,
  ) {
    return this.service.listLocations(levelOf(level), q).then(listResponse);
  }
  @Get('locations/:level/:uuid') @RequirePermissions('locations.read') get(
    @Param('level') level: string,
