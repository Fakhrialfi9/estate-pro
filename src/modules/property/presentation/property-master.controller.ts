...
const sanitize=(value:unknown):unknown=>{if(value instanceof Date)return value.toISOString();if(Array.isArray(value))return value.map(sanitize);if(value&&typeof value==='object'){const out:RecordValue={};for(const[k,v]of Object.entries(value)){if(k==='id'||k.endsWith('Id')||['createdBy','updatedBy','verifiedBy','deletedBy'].includes(k))continue;out[k]=sanitize(v);}return out;}return value;};
const response=(data:unknown)=>({data:sanitize(data)});
const listResponse=(r:{items:readonly unknown[];total:number;page:number;limit:number})=>({items:sanitize(r.items),meta:{page:r.page,limit:r.limit,total:r.total,totalPages:Math.ceil(r.total/r.limit)}});
