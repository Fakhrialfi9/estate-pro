  ...
  if (value && typeof value === 'object') {
    ...
    for (const [k, v] of Object.entries(value)) {
      if (
        k === 'id' ||
        k.endsWith('Id') ||
        ['createdBy', 'updatedBy', 'verifiedBy', 'deletedBy'].includes(k)
      )
        continue;
      out[k] = sanitize(v);
    }
...
export class PropertyMasterController {
  ...
  @Post('categories')
  @RequirePermissions('property-categories.create')
  ...
  listCategory(@Query() q: ListQuery) {
  ...
  listSubcategory(@Query() q: ListQuery) {
  ...
  async deleteSubcategory(
    ...
    await this.service.deleteSubcategory(uuid, actor(r, ua, rid));
  }
  @Post('locations/:level')
  ...
  getLocation(
    ...
    return this.service.getLocation(levelOf(level), uuid).then(response);
  }
  @Patch('locations/:level/:uuid')
  @RequirePermissions('locations.manage')
  updateLocation(
    @Req() r: AuthenticatedRequest,
    @Param('level') level: string,
    @Param('uuid', new ParseUUIDPipe({ version: '4' })) uuid: string,
    @Body() d: LocationUpdateDto,
    @Headers('user-agent') ua?: string,
    @Headers('x-request-id') rid?: string,
  ) {
    return this.service
  ...
  async deleteLocation(
    ...
    await this.service.deleteLocation(levelOf(level), uuid, actor(r, ua, rid));
  }
  @Get('locations/:level/:uuid/children')
  @RequirePermissions('locations.read')
  children(
    @Param('level') level: string,
    @Param('uuid', new ParseUUIDPipe({ version: '4' })) uuid: string,
  ...
  @RequirePermissions('facilities.update')
  updateFacility(
    @Req() r: AuthenticatedRequest,
    @Param('uuid', new ParseUUIDPipe({ version: '4' })) uuid: string,
