...
export interface PropertyExtrasRepository {
  ...
  getUtilities(propertyUuid: string): Promise<unknown | null>;
  upsertUtilities(
    propertyUuid: string,
    patch: UtilityPatch,
    actor: PropertyExtrasActor,
  ): Promise<unknown>;
  getLegal(propertyUuid: string): Promise<unknown | null>;
  upsertLegal(
    propertyUuid: string,
    patch: LegalPatch,
    actor: PropertyExtrasActor,
  ...
  getFinancial(propertyUuid: string): Promise<unknown | null>;
  upsertFinancial(
    propertyUuid: string,
    patch: FinancialPatch,
    actor: PropertyExtrasActor,
  ): Promise<unknown>;
  getFeatures(propertyUuid: string): Promise<unknown | null>;
  upsertFeatures(
    propertyUuid: string,
    patch: FeaturePatch,
    actor: PropertyExtrasActor,
  ): Promise<unknown>;
  getSecurity(propertyUuid: string): Promise<unknown | null>;
  upsertSecurity(
    propertyUuid: string,
    patch: SecurityPatch,
    actor: PropertyExtrasActor,
  ): Promise<unknown>;
  getEnvironment(propertyUuid: string): Promise<unknown | null>;
  upsertEnvironment(
    propertyUuid: string,
    patch: EnvironmentPatch,
    actor: PropertyExtrasActor,
  ): Promise<unknown>;
  getSeo(propertyUuid: string): Promise<unknown | null>;
  upsertSeo(
    propertyUuid: string,
    patch: SeoPatch,
    actor: PropertyExtrasActor,
