...
export const COORDINATE_ACCURACIES = [
  'ROOFTOP',
  'RANGE_INTERPOLATED',
  'GEOMETRIC_CENTER',
  'APPROXIMATE',
...
export const assertYearRange = (
  ...
  if (!Number.isInteger(value) || value < 1800 || value > year) {
    throw new PropertyDetailInvalidStateError(
      `${name} must be between 1800 and the current year`,
    );
  }
};
...
export const assertSpecificationInvariants = (input: {
  ...
  for (const [name, value] of Object.entries({
    ...
    maidRooms: input.maidRooms,
    guestToilets: input.guestToilets,
    parkingSpaces: input.parkingSpaces,
    livingRooms: input.livingRooms,
    familyRooms: input.familyRooms,
    diningRooms: input.diningRooms,
  ...
  if (
    input.floors !== undefined &&
    (!Number.isInteger(input.floors) || input.floors <= 0)
  ) {
    throw new PropertyDetailInvalidStateError(
  ...
  if (
    ...
    input.yearRenovated !== null &&
    input.yearRenovated !== undefined &&
    input.yearRenovated < input.yearBuilt
  ) {
    throw new PropertyDetailInvalidStateError(
      'yearRenovated must be greater than or equal to yearBuilt',
    );
  }
  ...
  if (building !== null && land !== null && building > land) {
    throw new PropertyDetailInvalidStateError(
      'buildingArea must not exceed landArea',
    );
  }
  ...
  if (parkingType === 'NONE' && parkingSpaces > 0) {
    throw new PropertyDetailInvalidStateError(
      'parkingSpaces must be zero when parkingType is NONE',
    );
  }
};

export const assertCoordinatePair = (
...
export const assertLocationHierarchy = (
  ...
  levels.forEach((value, index) => {
    ...
    if (value && missingFound)
      throw new PropertyDetailInvalidStateError(
        `${names[index]} cannot be supplied when a parent level is missing`,
      );
  });
};
...
export const assertPoolInvariants = (input: {
  ...
  if (
    ...
    throw new PropertyDetailInvalidStateError(
      'pool dimensions are only valid when hasPool is true',
    );
  }
  if (input.hasPool)
    dimensions.forEach((value, index) =>
      assertPositive(
        ['poolLengthM', 'poolWidthM', 'poolDepthM'][index],
        value as string | number,
      ),
    );
};

export const assertRoomInvariants = (input: {
  floor: number;
  area: string | number;
}): void => {
  if (!Number.isInteger(input.floor))
    throw new PropertyDetailInvalidStateError('floor must be an integer');
