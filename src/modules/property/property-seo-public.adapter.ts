...
import type { PropertySeoPublicAmenity, PropertySeoPublicPort, PropertySeoPublicSnapshot } from '../../common/contracts/property-seo-public.port.js';
...
export class PrismaPropertySeoPublicAdapter implements PropertySeoPublicPort {
  ...
  async getPublicProperty(uuidOrSlug: string): Promise<PropertySeoPublicSnapshot | null> {
    const property = await this.prisma.property.findFirst({
      ...
      select: {
        ...
        media: {
          ...
          select: { url: true, thumbnailUrl: true, type: true, isCover: true, sortOrder: true },
        ...
        amenityAssignments: {
          ...
          select: { amenity: { select: { code: true, name: true, category: true } } },
    ...
    const robots = property.seo?.robots ? String(property.seo.robots) : 'INDEX_FOLLOW';
    if (!INDEXABLE_ROBOTS.has(robots)) return null;

    const amenities: PropertySeoPublicAmenity[] = property.amenityAssignments.map(({ amenity }) => ({
      code: amenity.code,
      name: amenity.name,
    ...
    return {
      ...
      amenities,
      agent: property.agentAssignments[0]?.agentDisplayName ? { name: property.agentAssignments[0].agentDisplayName } : null,
    };
  }
}
