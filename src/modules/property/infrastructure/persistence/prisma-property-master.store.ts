...
export class PrismaPropertyMasterStore implements PropertyMasterRepository {
    ...
    try {
      return await this.prisma.$transaction(async (tx) => {
        ...
        if (parent) {
          ...
          if (!parentRecord) throw new MasterHierarchyError('Parent location not found or inactive');
  ...
  async updateLocation(level: string, uuid: string, version: number, patch: Row): Promise<unknown> {
    ...
    try {
      ...
      if (parent && typeof patch.parentUuid === 'string') {
        const parentRecord = await parent.delegate.findUnique({ where: { uuid: patch.parentUuid, deletedAt: null } });
        if (!parentRecord) throw new MasterHierarchyError('Parent location not found or inactive');
        data[parent.field] = row(parentRecord).id;
      }
  ...
  async listLocations(level: string, query: MasterQuery): Promise<PageResult<unknown>> {
    ...
    if (parent && query.parentUuid) {
      const p = await parent.delegate.findUnique({ where: { uuid: query.parentUuid, deletedAt: null } });
    ...
    try {
      await this.prisma.$transaction(async (tx) => {
        ...
        const childCounts: Record<string, number> = {
          country: await tx.province.count({ where: { countryId: row(item).id as bigint, deletedAt: null } }),
          province: await tx.city.count({ where: { provinceId: row(item).id as bigint, deletedAt: null } }),
          city: await tx.district.count({ where: { cityId: row(item).id as bigint, deletedAt: null } }),
          district: await tx.subdistrict.count({ where: { districtId: row(item).id as bigint, deletedAt: null } }),
          subdistrict: await tx.property.count({ where: { subdistrictId: row(item).id as bigint, deletedAt: null } }),
        };
  ...
  private async propertyRefs(tx: object, input: Row): Promise<{ typeId: bigint; categoryId: bigint; subcategoryId: bigint | null }> {
    ...
    const categoryRecord = await client.propertyCategory.findFirst({
      where: { uuid: text(input.categoryUuid), deletedAt: null, isActive: true },
    });
    if (!categoryRecord) throw new MasterHierarchyError('Property category not found or inactive');
    ...
    if (input.subcategoryUuid) {
      const subRecord = await client.propertySubcategory.findFirst({
        where: { uuid: text(input.subcategoryUuid), deletedAt: null, isActive: true },
      });
      if (!subRecord) throw new MasterHierarchyError('Property subcategory not found or inactive');
  ...
  async getProperty(uuid: string): Promise<unknown> {
    const r = await this.prisma.property.findFirst({ where: { uuid, deletedAt: null }, include: { propertyType: { select: { uuid: true, code: true, name: true } }, propertyCategory: { select: { uuid: true, code: true, name: true } }, propertySubcategory: { select: { uuid: true, code: true, name: true } }, subdistrict: { select: { uuid: true, code: true, name: true } }, facilities: { include: { facility: true } } } });
  ...
  async listProperties(q: MasterQuery): Promise<PageResult<unknown>> {
    ...
    const [items, total] = await Promise.all([
      this.prisma.property.findMany({ where, skip, take: limit, orderBy: orderOf(q, ['updatedAt', 'title', 'createdAt']), select: { uuid: true, businessCode: true, referenceNumber: true, title: true, slug: true, status: true, availabilityStatus: true, availableFrom: true, availableTo: true, version: true, createdAt: true, updatedAt: true, propertyType: { select: { uuid: true, code: true, name: true } }, propertyCategory: { select: { uuid: true, code: true, name: true } }, propertySubcategory: { select: { uuid: true, code: true, name: true } } } }),
  ...
  async updateProperty(uuid: string, version: number, patch: Row, actor: ActorContext): Promise<unknown> {
    try {
      ...
      const rowRecord = row(rowValue);
      if (rowRecord.version !== version) throw new MasterConcurrencyError('Property version conflict');
      const data: Row = { version: { increment: 1 }, updatedBy: actor.actorUuid ?? rowRecord.updatedBy };
      const title = optionalText(patch.title);
      const slug = optionalText(patch.slug);
      if (title !== null) data.title = title.trim();
      if (slug !== null || title !== null) data.slug = normalizeSlug(slug ?? title ?? text(rowRecord.slug));
      ...
      const from = (data.availableFrom as Date | null | undefined) ?? (rowRecord.availableFrom as Date | null);
      const to = (data.availableTo as Date | null | undefined) ?? (rowRecord.availableTo as Date | null);
      if (from && to && from > to) throw new MasterHierarchyError('availableFrom must not be later than availableTo');
      const updated = await this.prisma.property.updateMany({ where: { id: rowRecord.id, version }, data });
      if (updated.count !== 1) throw new MasterConcurrencyError('Property version conflict');
      return this.prisma.property.findUnique({ where: { id: rowRecord.id } });
    } catch (error: unknown) {
      mapError(error);
    }
  }
  ...
  async duplicateProperty(uuid: string, actor: ActorContext): Promise<unknown> {
    try {
      return await this.prisma.$transaction(async (tx) => {
        ...
        const copy = await tx.property.create({ data: { businessCode, referenceNumber, propertyTypeId: source.propertyTypeId, propertyCategoryId: source.propertyCategoryId, propertySubcategoryId: source.propertySubcategoryId, subdistrictId: source.subdistrictId, title: `${source.title} (Copy)`, slug: normalizeSlug(`${source.slug}-copy-${randomUUID().slice(0, 8)}`), shortDescription: source.shortDescription, description: source.description, status: 'DRAFT', availabilityStatus: source.availabilityStatus, availableFrom: source.availableFrom, availableTo: source.availableTo, version: 1, createdBy: actor.actorUuid ?? null, updatedBy: actor.actorUuid ?? null } });
        if (source.facilities.length) await tx.propertyFacility.createMany({ data: source.facilities.map((item) => ({ propertyId: copy.id, facilityId: item.facilityId })) });
        return copy;
      });
    } catch (error: unknown) {
      mapError(error);
