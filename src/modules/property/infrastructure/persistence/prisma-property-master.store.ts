...
export class PrismaPropertyMasterStore implements PropertyMasterRepository {
  ...
  async listCategories(query: MasterQuery): Promise<import('../../domain/property-master.types.js').PageResult<unknown>> {
  ...
  async listSubcategories(query: MasterQuery): Promise<import('../../domain/property-master.types.js').PageResult<unknown>> {
    const { page, limit, skip } = pageOf(query);
    const where: Row = { deletedAt: null };
    if (query.isActive !== undefined) where.isActive = query.isActive;
  ...
  private locationDelegate(level: string): Delegate {
    return this.delegate(
      this.prisma,
      ({ country: 'country', province: 'province', city: 'city', district: 'district', subdistrict: 'subdistrict' } as Record<string, string>)[level] ?? 'invalid',
    );
  }
  ...
  async listLocations(level: string, query: MasterQuery): Promise<import('../../domain/property-master.types.js').PageResult<unknown>> {
    const { page, limit, skip } = pageOf(query);
    const where: Row = { deletedAt: null };
    if (query.isActive !== undefined) where.isActive = query.isActive;
    const parent = this.parentConfig(level);
  ...
  async listFacilities(query: MasterQuery): Promise<import('../../domain/property-master.types.js').PageResult<unknown>> {
    const { page, limit, skip } = pageOf(query);
    const where: Row = { deletedAt: null };
    if (query.isActive !== undefined) where.isActive = query.isActive;
  ...
  private async propertyRefs(tx: object, input: Row): Promise<{ typeId: bigint; categoryId: bigint; subcategoryId: bigint | null }> {
    const client = tx as { propertyType: { findFirst(args: object): Promise<unknown> }; propertyCategory: { findFirst(args: object): Promise<unknown> }; propertySubcategory: { findFirst(args: object): Promise<unknown> } };
    ...
    if (type && category && type.id !== category.propertyTypeId) throw new MasterHierarchyError('Category does not belong to type');
    ...
    if (input.subcategoryUuid) {
      ...
      if (sub.propertyCategoryId !== category.id) throw new MasterHierarchyError('Subcategory does not belong to category');
  ...
  async getProperty(uuid: string): Promise<unknown> {
    const r = await this.prisma.property.findFirst({
      where: { uuid, deletedAt: null },
      include: { propertyType: { select: { uuid: true, code: true, name: true } }, propertyCategory: { select: { uuid: true, code: true, name: true } }, propertySubcategory: { select: { uuid: true, code: true, name: true } }, subdistrict: { select: { uuid: true, code: true, name: true } }, facilities: { include: { facility: true } } },
    });
    if (!r) throw new MasterNotFoundError('Property not found');
    return r;
  }
  async listProperties(q: MasterQuery): Promise<import('../../domain/property-master.types.js').PageResult<unknown>> {
  ...
  async updateProperty(uuid: string, version: number, patch: Row, actor: ActorContext): Promise<unknown> {
    try {
      const row = await this.prisma.property.findFirst({ where: { uuid, deletedAt: null } });
      if (!row) throw new MasterNotFoundError();
      if (row.version !== version) throw new MasterConcurrencyError('Property version conflict');
      const data: Row = { version: { increment: 1 }, updatedBy: actor.actorUuid ?? row.updatedBy };
      const title = optionalText(patch.title);
      const slug = optionalText(patch.slug);
      if (title !== null) data.title = title.trim();
      if (slug !== null || title !== null) data.slug = normalizeSlug(slug ?? title ?? row.slug);
      ...
      const from = (data.availableFrom as Date | null | undefined) ?? row.availableFrom;
      const to = (data.availableTo as Date | null | undefined) ?? row.availableTo;
      if (from && to && from > to) throw new MasterHierarchyError('availableFrom must not be later than availableTo');
      const updated = await this.prisma.property.updateMany({ where: { id: row.id, version }, data });
