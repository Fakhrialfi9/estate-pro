...
export const configuration = [appConfig, apiConfig, auditConfig, authConfig, corsConfig, databaseConfig, loggingConfig, observabilityConfig, rateLimitConfig, securityConfig];
...
export const configurationValidationSchema = Joi.object({
  ...
  AUTH_REFRESH_TOKEN_EXPIRES_IN: Joi.string().trim().pattern(/^(?:[7-9]|1\d|2\d|30)d$/).default('30d'),
  ...
  AUDIT_LOG_RETENTION_DAYS: Joi.number().integer().min(30).max(3650).default(365),
  AUDIT_LOG_USER_AGENT_MAX_LENGTH: Joi.number().integer().min(128).max(4096).default(1024),
