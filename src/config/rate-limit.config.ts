...
export const LOGIN_RATE_LIMIT = { ttl: Number(process.env.AUTH_LOGIN_RATE_LIMIT_TTL_MS ?? 60000), limit: Number(process.env.AUTH_LOGIN_RATE_LIMIT ?? 5) } as const;
export const REFRESH_RATE_LIMIT = { ttl: Number(process.env.AUTH_REFRESH_RATE_LIMIT_TTL_MS ?? 60000), limit: Number(process.env.AUTH_REFRESH_RATE_LIMIT ?? 10) } as const;
export const SECURITY_SESSION_RATE_LIMIT = { ttl: Number(process.env.SECURITY_SESSION_RATE_LIMIT_TTL_MS ?? 60000), limit: Number(process.env.SECURITY_SESSION_RATE_LIMIT_MAX ?? 30) } as const;
