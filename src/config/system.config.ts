...
export default registerAs('system', () => ({
  ...
  export: {
    ...
    maxArtifactBytes: Number(process.env.SYSTEM_EXPORT_MAX_ARTIFACT_BYTES ?? 25 * 1024 * 1024),
  ...
  webhook: {
    ...
    maxPayloadBytes: Number(process.env.SYSTEM_WEBHOOK_MAX_PAYLOAD_BYTES ?? 1024 * 1024),
