...
const authTokenResponse = objectSchema(
  {
    accessToken: schema('string', {
      description: 'Short-lived access token. Sensitive.',
      example: 'access-token-placeholder',
    }),
    tokenType: schema('string', { enum: ['Bearer'] }),
    expiresIn: schema('integer', { minimum: 1 }),
    refreshToken: schema('string', {
      description: 'Opaque rotating refresh token. Sensitive.',
      example: 'refresh-token-placeholder',
    }),
...
const mfaChallengeResponse = objectSchema(
  {
    ...
    challengeToken: schema('string', {
      description: 'Short-lived MFA challenge token. Sensitive.',
      example: 'mfa-challenge-placeholder',
    }),
    expiresIn: schema('integer', { minimum: 1 }),
  },
...
const nestedResponse = objectSchema(
  {
    data: schema('object', {
      ...
      description:
        'Serialized public property detail/extra resource. Exact fields are owned by the corresponding runtime serializer.',
    }),
  },
  ['data'],
);
...
const enrollmentResponse = objectSchema(
  {
    ...
    provisioningUri: schema('string', {
      description: 'TOTP provisioning URI. Sensitive provisioning material.',
    }),
...
const recoveryResponse = objectSchema(
  {
    recoveryCodes: arrayOf(
      schema('string', { description: 'Single-use recovery code. Sensitive.' }),
    ),
  },
  ['recoveryCodes'],
);
...
const addReusableResponses = (document: Document): void => {
  ...
  for (const [status, description] of Object.entries({
    ...
    document.components.responses[status] = apiResponse(
      description,
...
export const applyOpenApiContract = (
  ...
  if (document.info) {
    ...
    document.info.description =
      'Estate Pro public HTTP API. The generated OpenAPI contract follows runtime validation, serialization, authentication, authorization, and public error behavior.';
  }
  ...
  for (const [path, item] of Object.entries(document.paths ?? {})) {
    ...
    for (const method of METHODS) {
      ...
      if (normalized === '/auth/logout' && method === 'post') {
        setResponse(
          operation,
          201,
          ref('SuccessResponse'),
      ...
      if (normalized === '/auth/me' && method === 'get') {
        setResponse(
          operation,
          200,
          ref('UserResponse'),
      ...
      if (normalized === '/auth/sessions' && method === 'get') {
        addQueryParam(
          operation,
          'limit',
      ...
      if (normalized === '/auth/sessions/logout-all' && method === 'post') {
        setResponse(
          operation,
          201,
      ...
      if (/^\/auth\/sessions\/\d+$/.test(normalized) && method === 'delete') {
        addPathParam(
          operation,
          'id',
      ...
      if (normalized === '/auth/2fa' && method === 'get') {
        setResponse(
          operation,
          200,
      ...
      if (normalized === '/auth/2fa/enrollment' && method === 'post') {
        setResponse(
          operation,
          201,
      ...
      if (normalized === '/auth/2fa/enrollment/verify' && method === 'post') {
        setResponse(
          operation,
          201,
      ...
      if (normalized === '/auth/2fa/verify' && method === 'post') {
        operation.security = [];
        setResponse(
          operation,
          201,
      ...
      if (normalized === '/auth/2fa/disable' && method === 'post') {
        setResponse(
          operation,
          201,
          ref('SuccessResponse'),
      ...
      if (normalized === '/users' && method === 'get') {
        ...
        addQueryParam(
          ...
          'filterValue',
          schema('string', { maxLength: 100 }),
          'Filter value. For isActive use true or false.',
        );
        addQueryParam(
          operation,
          'sortBy',
      ...
      if (/^\/users\/username\/[^/]+$/.test(normalized) && method === 'get') {
        addPathParam(
          operation,
          'username',
          schema('string', { maxLength: 100 }),
      ...
      const typePath =
        ...
        /^\/property-types\/[^/]+$/.test(normalized);
      if (typePath) {
        ...
        if (method === 'get' && !uuidMatch) {
          ...
          addQueryParam(
            operation,
            'filterField',
            schema('string', { enum: ['code', 'name', 'slug', 'isActive'] }),
            'Filter field.',
          );
          addQueryParam(
            operation,
        ...
        if (method === 'get' && uuidMatch) {
          setResponse(
            operation,
            200,
            ref('PropertyTypeResponse'),
            'Property type returned.',
          );
          setErrors(operation, [400, 401, 403, 404, 500]);
        }
        if (method === 'post') {
      ...
      const catalogMatch = normalized.match(
        /^\/property\/(categories|subcategories|facilities)(?:\/([^/]+))?$/,
      );
      if (catalogMatch) {
        ...
        if (method === 'get' && !uuid) {
          ...
          if (catalogMatch[1] === 'categories')
            addQueryParam(
              operation,
              'typeUuid',
              schema('string', { format: 'uuid' }),
              'Property type UUID filter.',
            );
          setResponse(
            operation,
            200,
        ...
        if (method === 'patch') {
          setResponse(
            ...
            200,
            wrapped(ref('PropertyCatalogResponse')),
            `${catalogMatch[1]} updated.`,
          );
          setErrors(operation, [400, 401, 403, 404, 409, 500]);
        }
        if (method === 'delete') {
          setNoContent(operation, `${catalogMatch[1]} deleted.`);
          setErrors(operation, [400, 401, 403, 404, 409, 500]);
        }
        continue;
      }
      ...
      if (locationMatch) {
        ...
        if (method === 'get' && !uuid) {
          ...
          setResponse(
            ...
            objectSchema(
              {
                data: arrayOf(ref('PropertyLocationResponse')),
                meta: ref('PaginationMeta'),
              },
              ['data', 'meta'],
            ),
        ...
        if (method === 'get' && uuid && normalized.endsWith('/children')) {
          setResponse(
            operation,
            200,
            wrapped(arrayOf(ref('PropertyLocationResponse'))),
            'Child locations returned.',
          );
          setErrors(operation, [400, 401, 403, 404, 500]);
        ...
        if (method === 'delete') {
          setNoContent(operation, 'Location deleted.');
          setErrors(operation, [400, 401, 403, 404, 409, 500]);
        }
        continue;
      }

      if (normalized === '/property/properties' && method === 'get') {
        addListQuery(operation);
        addQueryParam(
          operation,
          'sortBy',
        ...
        setResponse(
          operation,
          200,
          ref('PropertyListResponse'),
          'Properties returned.',
        );
        setErrors(operation, [400, 401, 403, 500]);
        continue;
      }
      if (normalized === '/property/properties' && method === 'post') {
        setResponse(
          operation,
          201,
      ...
      if (propertyMatch) {
        ...
        if (/^\/(restore|duplicate)$/.test(suffix) && method === 'post') {
          setResponse(
            operation,
            201,
            ref('PropertyWrappedResponse'),
        ...
        if (/^\/(verify|publish)$/.test(suffix) && method === 'post') {
          setResponse(
            operation,
            201,
      ...
      if (/^\/health\/live$/.test(normalized) && method === 'get') {
        operation.security = [];
        setResponse(
          operation,
          200,
      ...
      if (/^\/health\/ready$/.test(normalized) && method === 'get') {
        operation.security = [];
        setResponse(
          operation,
          200,
