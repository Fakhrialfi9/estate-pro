...
const authTokenResponse = objectSchema(
  {
    accessToken: schema('string', { writeOnly: true, example: 'synthetic-access-token' }),
    tokenType: schema('string', { enum: ['Bearer'] }),
    expiresIn: schema('integer', { minimum: 1 }),
    refreshToken: schema('string', { writeOnly: true, example: 'synthetic-refresh-token' }),
...
const mfaChallengeResponse = objectSchema(
  {
    ...
    challengeToken: schema('string', { writeOnly: true, example: 'synthetic-mfa-challenge' }),
    expiresIn: schema('integer', { minimum: 1 }),
  },
...
const logoutAllResponse = objectSchema(
  { success: schema('boolean', { example: true }), revokedCount: schema('integer', { minimum: 0 }) },
...
const enrollmentResponse = objectSchema(
  {
    ...
    provisioningUri: schema('string', { writeOnly: true, example: 'otpauth://totp/synthetic' }),
...
const recoveryResponse = objectSchema(
  { recoveryCodes: arrayOf(schema('string', { writeOnly: true, example: 'SYNTHETIC-RECOVERY-CODE' })) },
  ['recoveryCodes'],
);
...
const addReusableResponses = (document: MutableDocument): void => {
  ...
  for (const [name, description] of Object.entries(definitions)) {
    document.components.responses[name] = apiResponse(description, ref('ApiErrorResponse'));
  }
};
  ...
  for (const [path, item] of Object.entries(document.paths ?? {})) {
    for (const method of METHODS) {
      ...
      if (/^\/api\/v1\/roles\/[^/]+$/.test(path)) {
        ...
        if (method === 'post') { setResponse(operation, 201, ref('RoleResponse'), 'Role created.'); setErrors(operation, [400, 401, 403, 409, 500]); }
      ...
      if (/^\/api\/v1\/roles\/[^/]+\/permissions\/[^/]+$/.test(path) && method === 'delete') {
        ...
        setResponse(operation, 200, ref('SuccessResponse'), 'Permission removed from role.');
      ...
      if (/^\/api\/v1\/users\/[^/]+\/roles$/.test(path)) {
        ...
        if (method === 'get') { setResponse(operation, 200, ref('RoleListResponse'), 'User roles returned.'); setErrors(operation, [400, 401, 403, 404, 500]); }
        if (method === 'post') { setResponse(operation, 201, rolePermissionAssignment, 'Role assigned to user.'); setErrors(operation, [400, 401, 403, 404, 409, 500]); }
      }
      if (/^\/api\/v1\/users\/[^/]+\/roles\/[^/]+$/.test(path) && method === 'delete') {
        addPathParam(operation, 'userUuid', schema('string', { format: 'uuid' }), 'Target user UUID.');
        addPathParam(operation, 'roleUuid', schema('string', { format: 'uuid' }), 'Role UUID.');
        setResponse(operation, 200, ref('SuccessResponse'), 'Role removed from user.');
        setErrors(operation, [400, 401, 403, 404, 409, 500]);
      }
      if (path === '/api/v1/permissions' && method === 'get') {
        addQueryParam(operation, 'page', schema('integer', { minimum: 1, default: 1 }), 'Page number.');
        addQueryParam(operation, 'limit', schema('integer', { minimum: 1, maximum: 100, default: 20 }), 'Maximum number of permissions.');
        ...
        addQueryParam(operation, 'sortDirection', schema('string', { enum: ['asc', 'desc'], default: 'desc' }), 'Sort direction.');
        setResponse(operation, 200, ref('PermissionListResponse'), 'Permissions returned.');
        setErrors(operation, [400, 401, 403, 500]);
      }
      if (/^\/api\/v1\/permissions\/[^/]+$/.test(path)) {
        ...
        if (method === 'post') { setResponse(operation, 201, ref('PermissionResponse'), 'Permission created.'); setErrors(operation, [400, 401, 403, 409, 500]); }
  ...
  for (const [path, item] of Object.entries(mutable.paths ?? {})) {
    ...
    for (const method of METHODS) {
      ...
      if (normalized === '/auth/login' && method === 'post') {
        ...
        setResponse(operation, 201, ref('SuccessResponse'), 'Current session revoked.');
        ...
        setQuery(operation, 'limit', schema('integer', { minimum: 1, maximum: 100, default: 20 }), 'Maximum number of sessions.');
        setQuery(operation, 'offset', schema('integer', { minimum: 0, default: 0 }), 'Zero-based offset.');
        setQuery(operation, 'includeInactive', schema('boolean', { default: false }), 'Whether to include inactive sessions.');
        setResponse(operation, 200, ref('SessionListResponse'), 'Own sessions returned.');
        setErrors(operation, [401, 429, 500]);
      } else if (normalized === '/auth/sessions/logout-all' && method === 'post') {
        setResponse(operation, 201, ref('LogoutAllResponse'), 'All own sessions revoked.');
        setErrors(operation, [401, 429, 500]);
      } else if (/^\/auth\/sessions\/\d+$/.test(normalized) && method === 'delete') {
        addPathParam(operation, 'id', schema('string', { pattern: '^\\d+$' }), 'Public numeric session identifier.');
        setResponse(operation, 200, ref('SuccessResponse'), 'Session revoked.');
        setErrors(operation, [400, 401, 404, 429, 500]);
      } else if (/^\/admin\/session-management\/users\/[^/]+\/sessions\/\d+\/revoke$/.test(normalized) && method === 'post') {
        addPathParam(operation, 'userUuid', schema('string', { format: 'uuid' }), 'Target user UUID.');
        addPathParam(operation, 'id', schema('string', { pattern: '^\\d+$' }), 'Public numeric session identifier.');
        setResponse(operation, 201, ref('SuccessResponse'), 'Target session revoked.');
        setErrors(operation, [400, 401, 403, 404, 429, 500]);
      } else if (normalized === '/auth/2fa' && method === 'get') {
        setResponse(operation, 200, ref('TwoFactorStatusResponse'), 'Two-factor status returned.');
        setErrors(operation, [401, 429, 500]);
      } else if (normalized === '/auth/2fa/enrollment' && method === 'post') {
        setResponse(operation, 201, ref('TwoFactorEnrollmentResponse'), 'Two-factor enrollment started.');
        setErrors(operation, [400, 401, 429, 500]);
      } else if (normalized === '/auth/2fa/enrollment/verify' && method === 'post') {
        setResponse(operation, 201, ref('TwoFactorEnabledRecoveryResponse'), 'Two-factor enrollment verified.');
        setErrors(operation, [400, 401, 429, 500]);
      } else if (normalized === '/auth/2fa/verify' && method === 'post') {
        operation.security = [];
        setResponse(operation, 201, ref('AuthTokenResponse'), 'MFA verification succeeded.');
        setErrors(operation, [400, 401, 429, 500]);
      } else if (normalized === '/auth/2fa/recovery-codes/regenerate' && method === 'post') {
        setResponse(operation, 201, ref('RecoveryCodesResponse'), 'Recovery codes regenerated.');
        setErrors(operation, [400, 401, 429, 500]);
      } else if (normalized === '/auth/2fa/disable' && method === 'post') {
        setResponse(operation, 201, ref('SuccessResponse'), 'Two-factor authentication disabled.');
        setErrors(operation, [400, 401, 429, 500]);
      ...
      } else if (/^\/users\/email\/[^/]+$/.test(normalized) && method === 'get') {
        addPathParam(operation, 'email', schema('string', { format: 'email' }), 'User email address.');
        setResponse(operation, 200, ref('UserResponse'), 'User returned.'); setErrors(operation, [400, 401, 403, 404, 500]);
      } else if (/^\/users\/username\/[^/]+$/.test(normalized) && method === 'get') {
        addPathParam(operation, 'username', schema('string', { maxLength: 100 }), 'Username.');
        setResponse(operation, 200, ref('UserResponse'), 'User returned.'); setErrors(operation, [400, 401, 403, 404, 500]);
        ...
        operation.security = [];
        setResponse(operation, 200, objectSchema({ status: schema('string', { enum: ['ok'] }), checks: objectSchema({ application: objectSchema({ status: schema('string', { enum: ['up'] }) }, ['status']) }, ['application']) }, ['status', 'checks']), 'Liveness check succeeded.');
      } else if (/^\/health\/ready$/.test(normalized) && method === 'get') {
        operation.security = [];
        setResponse(operation, 200, schema('object'), 'Readiness check succeeded.');
...
function setQuery(operation: Operation, name: string, value: Schema, description: string): void {
  addQueryParam(operation, name, value, description);
}
