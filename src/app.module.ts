...
@Module({
  imports: [
    ConfigModule.forRoot({ isGlobal: true, cache: true, load: configuration, validationSchema: configurationValidationSchema, validationOptions: { abortEarly: false, allowUnknown: true } }),
    LoggingModule,
    ThrottlerModule.forRootAsync({
      ...
      useFactory: (configService: ConfigService) => ({
        throttlers: [
          { name: 'default', ttl: configService.getOrThrow<number>('rateLimit.ttl'), limit: configService.getOrThrow<number>('rateLimit.limit') },
          { name: 'login', ttl: configService.getOrThrow<number>('rateLimit.login.ttl'), limit: configService.getOrThrow<number>('rateLimit.login.limit') },
        ],
